...
function detectUnit(text) {
  ...
  const restated = blob.match(
    /(?:SUMMARY OF FINANCIAL INFORMATION|as Restated|Financial Statements of (?:Assets|Profit)|FINANCIAL KPIs)[\s\S]{0,320}?(in\s*lakhs?|in\s*million|in\s*crore)/i
  );
  if (restated) {
  ...
  const lakhs = (blob.match(/in\s*lakhs?/gi) || []).length;
  const millions = (blob.match(/in\s*million/gi) || []).length;
  const crores = (blob.match(/in\s*crore/gi) || []).length;
...
function parseAbout(text) {
  ...
  if (companyIs && !/originally incorporated as “Company Limited/i.test(companyIs[0])) {
  ...
  const weAre = body.match(/We are (?:a |an |engaged)[\s\S]{40,700}?\.(?=\s+[A-Z“"']|\s*$)/i);
  ...
  const cut = sentence.length > 780 ? `${sentence.slice(0, 777).replace(/\s+\S*$/, "")}…` : sentence;
  return cut;
}
...
function parseOperations(text) {
  ...
  const m = blob.match(/Our store operations are structured around[\s\S]{40,480}?\.(?=\s+[A-Z]|\s*$)/i)
    ...
    || blob.match(/Our manufacturing facility is situated[\s\S]{40,420}?\.(?=\s+[A-Z]|\s*$)/i);
  if (!m) return null;
...
function isStrengthTitle(title) {
  ...
  if (/\b(in turn|including both|are deployed|have been|will be|such as)\b/i.test(title)) return false;
  const words = title.split(/\s+/).filter(Boolean);
  if (words.length < 3 || words.length > 12) return false;
  if (title.length < 18 || title.length > 120) return false;
  return true;
}
...
function parseRiskTitles(text) {
  ...
  const seen = new Set();
  const out = [];
  const re = /(\d{1,2})\.\s+((?:We|Our Company|Our |A substantial|The Company|Failure|Inability|Dependence)[\s\S]{20,240}?\.)/g;
  for (const m of slice.matchAll(re)) {
    const n = Number(m[1]);
    ...
    if (title.length < 30 || title.length > 220) continue;
...
function decimalPairAfter(labelRe, text) {
  const re = new RegExp(`${labelRe}[\\s\\S]{0,280}?`, "i");
  const start = String(text || "").search(re);
  if (start < 0) return null;
  const window = String(text).slice(start, start + 420);
  if (/\[●\]|\[\s*●\s*\]/.test(window.slice(0, 220))) return null;
  const nums = [...window.matchAll(/([\d,]+(?:\.\d{2}))/g)]
    .map((m) => parseAmount(m[1]))
    .filter((n) => n != null && n > 1 && (n < 1900 || n > 2100));
  if (!nums.length) return null;
  const amount = nums.find((n) => n >= 10) ?? nums[0];
  const pctLike = nums.find((n) => n !== amount && n < 100);
  if (pctLike != null && amount < 10) return null;
  return amount;
}
...
function parseObjectsOfOffer(text, freshCrore = null) {
  ...
  const intro = blob.match(
    /(?:proposes to utilise|intend to utilize) the Net Proceeds[\s\S]{0,80}?following objects:([\s\S]{0,1800}?)(?:collectively|Utilisation of Net Proceeds)/i
  );
  const purposes = [];
  if (intro) {
    const body = intro[1];
  ...
  const tableIdx = blob.search(/Utilisation of Net Proceeds/i);
  const table = tableIdx >= 0 ? blob.slice(tableIdx, tableIdx + 4500) : blob;
  ...
  for (const row of purposes) {
    if (/fit.?out|manufacturing facility|capital expenditure/i.test(row.purpose) && fitRaw != null) applyAmount(row, fitRaw);
    else if (/working capital/i.test(row.purpose) && wcRaw != null) applyAmount(row, wcRaw);
    else if (/general corporate/i.test(row.purpose)) {
