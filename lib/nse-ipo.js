...
function normalizeOpen(row) {
  ...
  const offered = nseNumber(row.noOfSharesOffered);
  const bookIsOpen = offered != null && offered > 0;
  return {
    ...normalizeUpcoming(row),
    status: row.status || "Active",
    overallSubscription: bookIsOpen && Number.isFinite(times) ? Number(times.toFixed(2)) : null,
    sharesOffered: offered,
    sharesBid: nseNumber(row.noOfsharesBid),
...
function normalizeListed(row) {
  ...
  return {
    ...
    lotSize: nseNumber(row.lotSize || row.marketLot || row.bidLot),
    issueSize: cleanNseText(row.issueSize),
...
function pickInfo(map, ...titles) {
  for (const title of titles) {
    ...
    const exact = Object.keys(map).find((key) => key.toLowerCase() === title.toLowerCase());
    if (exact && map[exact]) return map[exact];
  }
  for (const title of titles) {
    const needle = title.toLowerCase();
    if (needle.length < 6) continue;
    const fuzzy = Object.keys(map).find((key) => key.toLowerCase().includes(needle) && map[key]);
...
function extractDocumentLinks(dataList = []) {
  ...
  for (const row of dataList) {
    ...
    let key = labels[title];
    if (!key && /red herring prospectus|\brhp\b|^prospectus$/i.test(title)) key = "rhp";
    if (!key) key = title.toLowerCase().replace(/[^a-z0-9]+/g, "-");
    documents.push({ key, title, url: href.replace(/["']+$/, "") });
  }
  return documents;
}

function nseRhpArchiveUrl(symbol) {
  const s = String(symbol || "").trim().toUpperCase();
  if (!/^[A-Z0-9]{2,20}$/.test(s)) return null;
  return `https://nsearchives.nseindia.com/content/ipo/RHP_${s}.zip`;
}
...
function parseDemand(rows) {
  ...
  const levels = rows
    .map((row) => ({
      price: cleanNseText(row.price ?? row.type),
      cumulativeQty: cleanNseText(row.cumQty ?? row.value),
...
function priceBandFromDemand(rows) {
  if (!Array.isArray(rows) || rows.length < 2) return null;
  const prices = rows
    .map((row) => nseNumber(row?.price ?? row?.type))
    .filter((n) => n != null && n >= 1);
...
function parseIssueSnapshot(data, fallback = {}) {
  ...
  const demandBand = priceBandFromDemand(data?.demandDataNSE)
    || priceBandFromDemand(data?.demandDataBSE)
    || priceBandFromDemand(data?.demandGraph?.graphData);
  const fromInfo = pickInfo(map, "Price Range", "Price Band", "Issue Price", "Floor Price")
    || fallback.issuePrice
    || fallback.priceRange
    || meta.priceRange
    || meta.issuePrice
    || null;
  const price = parsePriceBand(fromInfo || demandBand?.display);
  const lotSize = firstNumber(pickInfo(map, "Bid Lot", "Minimum Order Quantity", "Lot Size", "Market Lot"))
    ?? nseNumber(fallback.lotSize)
    ?? nseNumber(meta.lotSize);
  const faceValue = firstNumber(pickInfo(map, "Face Value")) ?? nseNumber(meta.faceValue);
  const issueSizeText = pickInfo(map, "Issue Size", "Fresh Issue")
    || (typeof fallback.issueSize === "string" ? fallback.issueSize : null);
  let issueSizeCrore = parseIssueSizeCrore(issueSizeText);
  const sharesOffered = nseNumber(fallback.sharesOffered)
    ?? nseNumber(fallback.issueSize)
    ?? nseNumber(meta.issueSize);
  if (issueSizeCrore == null && sharesOffered != null && sharesOffered > 0 && price.high != null) {
    issueSizeCrore = Number(((sharesOffered * price.high) / 1e7).toFixed(2));
  }
  const minInvestment = lotSize != null && price.high != null
    ? Number((lotSize * price.high).toFixed(2))
    : nseNumber(fallback.minInvestment);
  ...
  return {
    ...
    priceMid: price.mid,
    priceSource: fromInfo ? "NSE issue info" : (price.display && demandBand ? demandBand.source : null),
    lotSize,
    faceValue,
    minInvestment,
    issueType: pickInfo(map, "Issue Type") || fallback.issueType || null,
    issueSizeText,
    issueSizeCrore,
    issueSizeDisplay: issueSizeCrore != null
      ? `₹${issueSizeCrore.toLocaleString("en-IN")} Cr`
      : (sharesOffered != null && sharesOffered > 0 ? `${sharesOffered.toLocaleString("en-IN")} shares` : null),
    sharesOffered,
...
function parseSubscriptionCategories(bidDetails = []) {
  ...
  const find = (match, { exact = false } = {}) => {
    ...
    const offered = nseNumber(use.noOfSharesOffered);
    const published = offered != null && offered > 0;
    return {
      available: published && formatted != null,
      value: published ? times : null,
      display: published && formatted != null ? `${formatted}x` : null,
      sharesOffered: offered,
  ...
  const overallOffered = nseNumber(total?.noOfSharesOffered);
  const overallFmt = formatSubscription(overallTimes);
  const bookIsOpen = overallOffered != null && overallOffered > 0;
  const niiExact = find("Non Institutional Investors", { exact: true });

  return {
    overall: {
      available: bookIsOpen && overallFmt != null,
      value: bookIsOpen ? overallTimes : null,
      display: bookIsOpen && overallFmt != null ? `${overallFmt}x` : null,
      sharesOffered: overallOffered,
      sharesBid: nseNumber(total?.noOfsharesBid),
      source: "NSE ipo-detail",
      reason: bookIsOpen ? null : "Bid book has not published shares offered yet — 0x is not a real subscription print",
    },
...
function subscriptionIsLive(sub) {
  return sub?.overall?.available === true && nseNumber(sub.overall.sharesOffered) > 0;
}

function subscriptionFromListRow(ipo) {
  if (nseNumber(ipo?.sharesOffered) > 0 && ipo.overallSubscription != null) {
    return {
      overall: {
        available: true,
        value: ipo.overallSubscription,
        display: `${Number(ipo.overallSubscription).toFixed(2)}x`,
        sharesOffered: nseNumber(ipo.sharesOffered),
        sharesBid: nseNumber(ipo.sharesBid),
        source: ipo.source || "NSE ipo-current-issue",
        reason: null,
      },
    };
  }
...
function pickBetterSubscription(detailSub, ipo) {
  if (subscriptionIsLive(detailSub)) return detailSub;
  const fromList = subscriptionFromListRow(ipo);
  if (fromList) {
    return { ...(detailSub || {}), ...fromList, categories: detailSub?.categories || fromList.categories };
  }
  return detailSub || ipo?.subscription || null;
}
...
function mergeIpoDetail(ipo, detail) {
  ...
  const filledSnap = {
    ...
    priceRange: snap.priceRange || ipo.issuePrice || ipo.priceRange || null,
    ...
    issueSizeDisplay: snap.issueSizeDisplay || ipo.issueSize || null,
    issueStartDate: snap.issueStartDate || ipo.issueStartDate || null,
    issueEndDate: snap.issueEndDate || ipo.issueEndDate || null,
    faceValue: snap.faceValue ?? ipo.faceValue ?? null,
    minInvestment: snap.minInvestment ?? ipo.minInvestment ?? null,
  };
  if (filledSnap.issueSizeCrore == null && nseNumber(filledSnap.sharesOffered) > 0 && filledSnap.priceHigh != null) {
    filledSnap.issueSizeCrore = Number(((Number(filledSnap.sharesOffered) * Number(filledSnap.priceHigh)) / 1e7).toFixed(2));
    filledSnap.issueSizeDisplay = `₹${filledSnap.issueSizeCrore.toLocaleString("en-IN")} Cr`;
  }
  return {
    ...ipo,
    companyName: filledSnap.companyName,
    issuePrice: filledSnap.priceRange || ipo.issuePrice,
    issueSize: filledSnap.issueSizeDisplay || ipo.issueSize,
    issueSizeCrore: filledSnap.issueSizeCrore ?? null,
    issueSizeText: snap.issueSizeText || ipo.issueSizeText || null,
    sharesOffered: filledSnap.sharesOffered,
    lotSize: filledSnap.lotSize,
    faceValue: filledSnap.faceValue,
    minInvestment: filledSnap.minInvestment,
    leadManagers: snap.leadManagers || ipo.leadManagers || null,
    registrar: snap.registrar || ipo.registrar || null,
    sponsorBank: snap.sponsorBank || ipo.sponsorBank || null,
    issueType: snap.issueType || ipo.issueType || null,
    industry: detail.metaInfo?.industry || ipo.industry || null,
    listingDate: detail.metaInfo?.listingDate || ipo.listingDate || null,
    isin: detail.metaInfo?.isin || ipo.isin || null,
    documents: (detail.documents && detail.documents.length) ? detail.documents : (ipo.documents || []),
    subscription: pickBetterSubscription(detail.subscription, ipo),
    demand: detail.demand || ipo.demand || null,
    issueSnapshot: filledSnap,
  };
}
...
async function fetchIpoDetail(symbol, fallback = {}) {
  ...
  const issueSnapshot = parseIssueSnapshot(data, { ...fallback, symbol });
  return {
    symbol,
    companyName: issueSnapshot.companyName || data.companyName || fallback.companyName || symbol,
    metaInfo: data.metaInfo || {},
    issueSnapshot,
    documents: extractDocumentLinks(data.issueInfo?.dataList || []),
    demand: parseDemand(data.demandDataNSE || data.demandDataBSE),
    subscription,
    bidDetails,
...
async function enrichWithDetail(ipos) {
  return Promise.all(
    (ipos || []).map(async (ipo) => {
      try {
        const detail = await fetchIpoDetail(ipo.symbol, ipo);
...
async function fetchRhpArchive(url, timeoutMs = 90_000) {
  try {
    ...
    if (res.ok) {
      ...
      if (buf.length > 100 && (magic === "PK" || buf.slice(0, 4).toString("utf8") === "%PDF")) {
        return buf;
      }
    }
  } catch {
    // Cookie-warmed NSE session is the fallback when the archive CDN rejects a bare GET.
  }
  return fetchNseBinary(url, timeoutMs);
}

async function fetchNseBinary(url, timeoutMs = 60_000) {
  let lastError;
  for (let i = 0; i < 3; i++) {
    try {
      if (i > 0) {
      ...
      const buf = Buffer.from(await res.arrayBuffer());
      if (buf.length < 100) throw new Error("NSE archive was empty");
      const magic = buf.slice(0, 2).toString("utf8");
      if (magic !== "PK" && buf.slice(0, 4).toString("utf8") !== "%PDF") {
        throw new Error("NSE archive was not a zip or PDF");
      }
      return buf;
    } catch (err) {
      lastError = err;
