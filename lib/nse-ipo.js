...
function parseIssueSnapshot(data, fallback = {}) {
  ...
  const price = parsePriceBand(
    pickInfo(map, "Price Range", "Price Band", "Issue Price", "Floor Price")
    || fallback.issuePrice
    || fallback.priceRange
    || meta.priceRange
    || meta.issuePrice
  ...
  const issueSizeText = pickInfo(map, "Issue Size", "Fresh Issue") || fallback.issueSize || null;
  const issueSizeCrore = parseIssueSizeCrore(issueSizeText);
  const sharesOffered = nseNumber(fallback.sharesOffered || fallback.issueSize || meta.issueSize);
