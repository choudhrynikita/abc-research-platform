...
function ticketFor(row, spec, side, native, mcx, proxy, reason) {
  ...
  if (proxy?.price != null && side !== "sell") {
    steps.push(`No MCX login? BUY ${row.proxy.replace(".NS", "")} CNC around ₹${Number(proxy.price).toLocaleString("en-IN")} instead — that is an overlay, not this futures ticket.`);
  } else if (proxy?.price != null && side === "sell") {
    steps.push(`Do not add ${row.proxyName}. Trim an existing BeES overweight. Do not short the ETF unless you understand STBT.`);
  }
  return { action, steps };
}
...
function buildPlans(row, latest, trend, price, ctx = {}) {
  ...
  if (violent && row.id === "natgas") {
    ...
    status = "Pass";
    reason = "Natural gas gaps several percent. Default is no new lot.";
  } else if (row.id === "usdinr" && trend === "BULLISH" && price != null) {
    side = "buy";
    structure = "Importer hedge";
    name = "BUY 1 lot USDINR — rupee weakening, importers hedge";
    status = "Plan";
  } else if (row.id === "usdinr" && trend === "BEARISH" && price != null) {
    side = "sell";
  ...
  const fillSheet = spec
    ? mcxSheet(row, spec, side, mcx, { stop: stopLoss, heat }, ticket)
    : null;
  ...
  if (row.proxy && proxy?.price != null) {
    ...
    const etfSteps = trend === "BEARISH"
      ? [
          `Open NSE cash. Search ${row.proxy.replace(".NS", "")}. Do not BUY today — metal tape is bearish.`,
      ...
      : [
          `Open NSE cash. Search ${row.proxy.replace(".NS", "")}. Product type: CNC.`,
          `BUY ${units} units (₹5,000 example) at limit ₹${Number(proxy.price).toLocaleString("en-IN")}.`,
          "This is a 5–10% gold/silver overlay, not a day trade and not GOLDMINI.",
    ...
    plans.push({
      ...
      last: proxy.price,
      tradeLine: `${etfSide} ${row.proxy.replace(".NS", "")} · ${units} units @ ₹${Number(proxy.price).toLocaleString("en-IN")} · CNC`,
      tradeTicket: { action: etfSide, steps: etfSteps },
      fillSheet: {
        venue: "NSE cash",
        product: row.proxy.replace(".NS", ""),
        side: etfSide,
        ...
        path: `NSE → ${row.proxy.replace(".NS", "")} → CNC → ${etfSide}`,
      },
      why: [
