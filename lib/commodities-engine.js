...
function beesDisagreement(row, mcxLast, beesPrice) {
  const fromBees = beesToMcx(row.id, beesPrice);
  if (fromBees == null || mcxLast == null || Number(mcxLast) === 0) return null;
  const pct = round(((fromBees - mcxLast) / mcxLast) * 100, 2);
  return {
    fromBees,
    pct,
    absPct: Math.abs(pct),
    trusted: Math.abs(pct) <= 2,
  };
}
...
function beesCheckLine(row, mcxLast, beesPrice) {
  ...
  const warn = gap != null && Math.abs(gap) > 2
    ? " — do not send the COMEX rupee map; confirm MCX LTP"
    : "";
  return `${row.proxyName || "BeES"} ${px} ⇒ ${unit} ≈ ₹${inr(fromBees)}${gapTxt}${warn}`;
}
...
function ticketFor(row, spec, side, native, mcx, proxy, reason) {
  ...
  const mapOk = native.mapOk !== false;
  if (side === "pass") {
  ...
  } else if (mcx.last != null && !mapOk) {
    const gap = native.beesGapPct != null ? ` disagrees by ${native.beesGapPct}%` : " disagrees";
    steps.push(`${action} 1 lot after you read ${spec.code} LTP. COMEX × USDINR maps to ₹${inr(mcx.last)} / ${spec.quote} but ${ticker}${gap}. Do not type that rupee number into the ticket.`);
  } else {
  ...
  if (mcx.stop != null && mapOk) steps.push(`Hard stop: ₹${Number(mcx.stop).toLocaleString("en-IN")} per ${spec.quote}. Not a mental note.`);
  else if (native.stop != null) steps.push(`Hard stop: ${formatNative(row, native.stop)} on the dollar tape, mirrored onto the rupee lot after you read LTP.`);
  if (mcx.t1 != null && mapOk) {
...
function mcxSheet(row, spec, side, mcx, native, ticket, extra = {}) {
  ...
  const mapOk = extra.mapOk !== false;
  const sendable = !pass && mapOk && rupee;
  return {
    venue,
    product: spec.code,
    side: action,
    ...
    limit: pass ? "Do not send" : (sendable && mcx.entry ? fmtZone(mcx.entry, "₹") : "Confirm MCX LTP"),
    stop: pass
      ? "Do not send"
      : sendable && mcx.stop != null
        ? `₹${Number(mcx.stop).toLocaleString("en-IN")}`
        : native.stop != null
          ? `${formatNative(row, native.stop)} on COMEX — write the MCX stop after LTP`
          : "Confirm LTP then write the ATR stop",
    target: pass
      ? "None — no new lot"
      : sendable && mcx.t1 != null
    ...
    mcxEstimate: mcx.last != null
      ? (mapOk
        ? `₹${inr(mcx.last)} (${spec.quote})`
        : `₹${inr(mcx.last)} COMEX map — do not send; BeES disagrees ${extra.beesGapPct != null ? `${extra.beesGapPct}%` : ""}`.trim())
      : "Confirm MCX LTP",
...
function buildPlans(row, latest, trend, price, ctx = {}) {
  ...
  const disagreement = beesDisagreement(row, mcxLast, proxy?.price);
  const mapOk = disagreement ? disagreement.trusted : true;

  if (side !== "pass" && spec && mapOk && mcx.entry?.low != null) {
  ...
  } else if (side !== "pass" && spec && !mapOk) {
    const tag = side === "buy" ? "dip in uptrend" : "fade in downtrend";
    name = `${side === "sell" ? "SELL" : "BUY"} 1 lot ${spec.code} — ${tag} · confirm MCX LTP`;
  }
  ...
  const tradeLine = side === "pass"
    ...
    : mapOk
      ...
      : `${ticket.action} 1 lot ${spec.code} · confirm MCX LTP (COMEX map and BeES disagree)`;
  ...
  const why = [
    ...
    !mapOk && disagreement
      ? { category: "Fill", text: `COMEX × USDINR and ${row.proxyName || "BeES"} disagree by ${disagreement.pct}%. Direction from the SMA stack still stands. The rupee limit does not — confirm MCX LTP.` }
      : null,
  ...
  plans.push({
    ...
    lots: side === "pass" ? 0 : 1,
    entryZone: side === "pass" ? "Do not send" : (mapOk && mcx.entry ? mcx.entry : "Confirm MCX LTP"),
    stopLoss: side === "pass"
      ? "Do not send"
      : mapOk && mcx.stop != null
        ? mcx.stop
        : (stopLoss != null ? "Confirm LTP then 1.5× ATR stop" : "Confirm LTP"),
    targets: side === "pass"
      ? { t1: "None — no new lot", t2: null }
      : mapOk
        ? { t1: mcx.t1 ?? t1, t2: mcx.t2 ?? t2 }
        : { t1: "Confirm LTP", t2: null },
...
function assembleDesk(rows, extras = {}) {
  ...
  for (const row of rows) {
    for (const plan of row.plans || []) {
      strategies.push({
        ...plan,
        last: plan.last ?? row.mcxEstimate ?? row.price,
        trend: plan.bias === "Bullish" ? "BULLISH" : plan.bias === "Bearish" ? "BEARISH" : "NEUTRAL",
      });
    }
  }
  ...
  return {
    ...
    notes: [
      ...
      "If Gold/Silver BeES and COMEX × USDINR disagree by more than 2%, the card keeps the SMA direction but will not print a sendable rupee limit.",
