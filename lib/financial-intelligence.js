...
const {
  ...
  SYSTEM_PROMPT,
  ASSISTANT_PREAMBLE,
} = require("./financial-intelligence-prompt");

/** Canonical user-facing messages — never substitute with estimates. */
const MESSAGES = {
  UNAVAILABLE_CURRENT: "Live Data Currently Unavailable",
  UNAVAILABLE_GENERAL: "Data Unavailable",
  ...
  AWAITING: "Awaiting Latest Verified Data",
  SOURCE_MISSING: "Source Does Not Provide This Information",
};
...
/**
 * Full canonical system prompt for AI assistants and policy endpoints.
 */
...
/**
 * Policy metadata for API _meta blocks and assistant context.
 */
...
/**
 * Enforce cache TTL — expired cache must be refreshed, never served as current.
 */
function enforceFreshDataPolicy(collectedAt, maxAgeMs = DEFAULT_CACHE_TTL_MS) {
  const ts = validateTimestamp(collectedAt, { maxAgeMs });
  if (!ts.valid) {
    return {
      fresh: false,
      stale: collectedAt != null,
      reason: ts.reason || MESSAGES.UNAVAILABLE_CURRENT,
      message: MESSAGES.UNAVAILABLE_CURRENT,
    };
  }
  return { fresh: true, stale: false, reason: null, message: null, collectedAt: ts.parsedAt };
}

/**
 * Institutional figure — value with mandatory provenance fields per website quality standards.
 */
function buildInstitutionalFigure(opts) {
  const field = buildVerifiedField(opts);
  if (!field.verified) return field;
  return {
    ...field,
    reportingPeriod: opts.period || null,
    dataSource: opts.source || null,
    reproducible: true,
  };
}

/**
 * Map transparency label for mixed factual / opinion API payloads.
 */
function classifyDataType({ verified = false, isModel = false, isForecast = false, isEducational = false } = {}) {
  if (!verified) return DATA_CLASSIFICATION.UNAVAILABLE;
  if (isForecast) return DATA_CLASSIFICATION.FORECAST;
  if (isModel) return DATA_CLASSIFICATION.ANALYSIS;
  if (isEducational) return DATA_CLASSIFICATION.OPINION;
  return DATA_CLASSIFICATION.VERIFIED_FACT;
}
