...
function buildVerifiedSnapshot(strategy, marketContext, derivativesIntel, module, refreshedAt) {
  ...
  return {
    ...
    maxRisk: strategy?.maxRisk ?? strategy?.payoff?.maxLoss ?? null,
    maxReward: strategy?.maxReward ?? strategy?.payoff?.maxProfit ?? null,
    maxProfitUnlimited: strategy?.payoff?.maxProfitUnlimited === true,
    maxLossUnlimited: strategy?.payoff?.maxLossUnlimited === true,
    riskRewardRatio: strategy?.payoff?.maxProfitUnlimited || /^(Long CE|Long PE)$/i.test(strategy?.type || "")
      ? null
      : strategy?.riskRewardRatio ?? null,
    breakEven: strategy?.payoff?.breakEvenDisplay ?? ps?.breakEven ?? null,
...
function getEducationalInsights(strategy) {
  ...
  return {
    ...
    limitations: [
      ...
      strategy?.mode && strategy.mode !== "live"
        ? "Reference plans use the latest verified close — confirm the premium and trigger when the planned session begins."
        : null,
    ].filter(Boolean),
...
function answerLiveMetric(intent, snap) {
  ...
  if (intent === "live_price") {
    ...
    push("Net premium", snap.netPremium != null ? fmtRs(Math.abs(snap.netPremium)) : null, snap.mode && snap.mode !== "live" ? "reference close" : "NSE chain");
...
function answerCalculation(intent, snap, strategy) {
  ...
  if (intent === "calculation" || intent === "max_loss") {
    sections.push(
      "**Max loss:** Defined-risk = debit paid, or wing width minus credit. Naked short calls are theoretically unlimited."
    );
    if (snap.maxLossUnlimited) {
      sections.push("Verified max loss: **Unlimited** (naked short call exposure).");
    } else if (snap.maxRisk != null) {
      sections.push(`Verified max loss: ${fmtRs(snap.maxRisk)} per unit from expiry payoff on verified premiums.`);
    } else {
      sections.push(`Max risk: ${NA_MSG}`);
    }
  }

  if (intent === "calculation" || intent === "max_profit") {
    if (snap.maxProfitUnlimited) {
      sections.push(
        "**Max profit:** Theoretically **unlimited** (net long calls). That is not a price target — size using the premium at risk."
      );
    } else if (/^Long PE$/i.test(snap.strategyType || "") && snap.maxReward != null) {
      sections.push(
        `**Max profit:** Theoretical if the underlying goes to zero: ${fmtRs(snap.maxReward)} per unit. That is **not** a realistic NIFTY/stock target. Use the strategy's T1/T2 management levels, not this theoretical figure.`
      );
    } else if (snap.maxReward != null) {
      sections.push(
        `**Max profit:** ${fmtRs(snap.maxReward)} per unit — spread width minus net debit, or credit received, from verified NSE premiums.`
      );
    } else {
      sections.push(`Max reward: ${NA_MSG}`);
    }
  }
...
function answerStrategyQuestion({
  ...
  switch (intent) {
    ...
    case "max_loss":
      answer = paragraph([
        "**Maximum Loss**",
        snap.maxLossUnlimited
          ? "Verified max loss is **unlimited** (naked short call). Size as if the entire account can be hit."
          : snap.maxRisk != null
            ? `Verified maximum defined risk: **${fmtRs(snap.maxRisk)} per unit** from the expiry payoff on NSE premiums.`
            : `${NA_INSUFFICIENT} Max loss is not verified — common for reference plans awaiting premium confirmation.`,
    ...
    case "max_profit":
      answer = paragraph([
        "**Maximum Profit**",
        snap.maxProfitUnlimited
          ? "Theoretically **unlimited** (net long calls / long straddle upside). That is not a price target — the premium at risk is the real budget."
          : /^Long PE$/i.test(snap.strategyType || "") && snap.maxReward != null
            ? `Theoretical max if the underlying goes to **zero**: ${fmtRs(snap.maxReward)} per unit. That is **not** a realistic target for NIFTY or large-cap stocks. Use T1/T2 management levels.`
            : snap.maxReward != null
              ? `Verified max reward: **${fmtRs(snap.maxReward)} per unit** (spread width − debit, or credit received).`
