...
async function buildNifty500Report() {
  ...
  const executiveSummary = `NIFTY 500 proxy at ${fmt(o.price)} (${fmtPct(o.dailyChangePercent)} daily). Trend: ${t.trend}. Breadth: ${breadth.advances} advances vs ${breadth.declines} declines across ${breadth.sampleSize} of ${breadth.totalTracked} sample constituents with verified quotes.`;
  ...
  return {
    type: "nifty500",
    title: `NIFTY 500 Market Intelligence (Sample Universe) — ${new Date().toISOString().slice(0, 10)}`,
    source: "Yahoo Finance Chart API + NSE reference constituents",
    generatedAt: new Date().toISOString(),
    dataFreshness: data.dataFreshness,
    confidence,
    disclaimer: "Index uses ^CRSLDX proxy. Constituent analysis covers a limited reference sample — not the full NIFTY 500. Fundamentals marked unavailable where feed unavailable — never estimated.",
    sections,
    dashboard: data,
  };
}
