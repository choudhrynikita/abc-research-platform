...
/**
 * Directional underlying proxy backtest for options bias alignment.
 * Uses verified OHLCV of the underlying only — NOT historical option premiums.
 * Clearly labeled so it is never presented as multi-leg P&L.
 *
 * Rules (documented):
 *  Bullish bias proxy: long underlying when close > SMA20, SMA20 > SMA50, RSI 45–70
 *  Bearish bias proxy: short-proxy (inverse long) when close < SMA20, SMA20 < SMA50, RSI 30–55
 *  Neutral bias: no trades — unavailable
 *  Exit: opposite structure, RSI extreme, ATR stop, or maxHold bars
 */
function backtestDirectionalProxy(candles, bias = "Bullish", opts = {}) {
  const direction =
    String(bias || "").toLowerCase() === "bearish"
      ? "bearish"
      : String(bias || "").toLowerCase() === "bullish"
        ? "bullish"
        : "neutral";
  ...
  if (direction === "neutral") {
    return {
      available: false,
      reason:
        "Underlying directional proxy not applicable for Neutral / non-directional options structures",
      samples: 0,
      proxyType: "underlying-directional",
      disclaimer:
        "This is not a multi-leg options premium backtest. Historical option premiums are unavailable from free verified feeds.",
    };
  }

  // Reuse SMA trend engine for bullish; invert returns for bearish proxy
  const base = backtestSmaTrend(candles, opts);
  if (!base.available) {
    return {
      ...base,
      proxyType: "underlying-directional",
      bias: direction,
      disclaimer:
        "Underlying directional proxy only — not multi-leg option P&L. Premium series unavailable.",
      assumptions: [
        ...(base.assumptions || []),
        `Proxy bias: ${direction}`,
        "Does not model option theta, gamma, or premium decay",
      ],
    };
  }
  ...
  if (direction === "bullish") {
    return {
      ...
      bias: "bullish",
      methodology:
        "Long underlying when SMA20>SMA50, price>SMA20, RSI 45–70 — proxy for bullish options bias alignment",
      disclaimer:
        "Underlying directional proxy on verified OHLCV — NOT historical multi-leg option premium P&L. Past proxy performance is not a guarantee.",
      assumptions: [
        ...(base.assumptions || []),
        "Does not model option premiums, Greeks, or gap risk on option legs",
      ],
    };
  }

  // Bearish: invert each trade return (short-proxy on same entry rules flipped)
  // Re-run with inverted entry logic by flipping close series relative to trend
  // Simpler honest approach: invert returns of bullish rule as approximation of short side
  // Better: custom loop for bearish entries
  const bearish = backtestSmaTrendBearish(candles, opts);
  return {
    ...bearish,
    proxyType: "underlying-directional",
    bias: "bearish",
    methodology:
      "Short-proxy on underlying when SMA20<SMA50, price<SMA20, RSI 30–55 — proxy for bearish options bias",
    disclaimer:
      "Underlying directional proxy on verified OHLCV — NOT historical multi-leg option premium P&L. Past proxy performance is not a guarantee.",
  };
}

/** Bearish mirror of SMA trend rule on verified OHLCV */
function backtestSmaTrendBearish(candles, { maxHold = 20, minTradesForStats = 5 } = {}) {
  if (!Array.isArray(candles) || candles.length < 80) {
    return {
      available: false,
      reason: "Awaiting Latest Verified Data — need ≥80 daily bars for bearish proxy backtest",
      samples: 0,
      trades: [],
    };
  }
  const closes = candles.map((c) => c.close).filter((c) => c != null && Number.isFinite(c));
  if (closes.length < 80) {
    return { available: false, reason: "Verified close series incomplete", samples: 0, trades: [] };
  }
  ...
  for (let i = 55; i < closes.length; i++) {
    ...
    if (open) {
      ...
      if (stop != null && px >= stop) {
        exitPx = stop;
        reason = "ATR stop (short)";
      } else if (sma20 != null && px > sma20) {
        exitPx = px;
        reason = "Close above SMA20";
      ...
      if (exitPx != null) {
        ...
        const retPct = ((open.entry - exitPx) / open.entry) * 100;
        trades.push({
          entryDate: open.entryDate,
          exitDate: date,
          entry: Number(open.entry.toFixed(2)),
  ...
  if (trades.length < minTradesForStats) {
    return {
      available: false,
      reason: `Bearish proxy produced only ${trades.length} closed trades — need ≥${minTradesForStats} (never fabricate)`,
      samples: trades.length,
      trades: trades.slice(-20),
      rules: [
        "Short-proxy when close < SMA20, SMA20 < SMA50, RSI 30–55",
        "Exit on close > SMA20, RSI < 25, ATR stop (1.5×), or max hold 20",
      ],
      assumptions: ["Underlying short-proxy only", "No option premium modeling"],
    };
  }
  ...
  return {
    ...
    lossRate: Number(lossRate.toFixed(1)),
    averageReturnPct: Number(avgReturn.toFixed(2)),
    averageWinPct: wins.length ? Number((wins.reduce((a, b) => a + b, 0) / wins.length).toFixed(2)) : null,
    ...
    sortinoRatio: null,
    numberOfTrades: trades.length,
    period: { from: trades[0]?.entryDate || null, to: trades[trades.length - 1]?.exitDate || null },
    rules: [
      "Short-proxy when close < SMA20, SMA20 < SMA50, RSI 30–55",
      "Exit on close > SMA20, RSI < 25, ATR stop (1.5×), or max hold 20",
    ],
    assumptions: [
      "Underlying short-proxy only — not option premium P&L",
      "Verified Yahoo/NSE daily OHLCV",
      "No brokerage or slippage modeled",
    ],
    trades: trades.slice(-25),
    disclaimer:
      "Underlying directional proxy — not multi-leg options backtest. Past performance is not a guarantee.",
  };
}
