...
async function buildInstitutionalStrategyDashboard() {
  const [history, chain, breadthData, vix, fiiDii] = await Promise.all([
  ...
  const price = prediction.currentPrice ?? chain?.underlying ?? candles.at(-1)?.close;
  ...
  const candidates = generateCandidates(chain, context);
  const top10 = rankTop10(candidates, context);
  const insights = buildInsights(top10, context);
  ...
  const executiveSummary = {
    ...
    chainVerified: chain?.available === true,
  ...
  return {
    ...
    subtitle: "Top 10 highest-conviction NIFTY options strategies — NIFTY weekly & monthly only",
    refreshedAt: new Date().toISOString(),
    source: chain?.available
      ? "NSE option-chain-indices + Yahoo Finance + ABC quantitative engine"
      : "Yahoo Finance (options chain unavailable — limited strategies)",
    executiveSummary,
    marketContext: {
      niftyTrend: trend,
      spotPrice: price,
    ...
    chainStatus: chain?.available
      ? { verified: true, expiry: chain.expiry, expiries: chain.expiries?.slice(0, 4), fetchedAt: chain.fetchedAt }
      : { verified: false, message: chain?.reason || "NSE NIFTY option chain unavailable after retries" },
