...
async function buildInstitutionalStrategyDashboard() {
  ...
  const [history, liveChain, monthlyPack, breadthData, vix, fiiDii] = await Promise.all([
    fetchNiftyHistory("2y").catch(() =>
      fetchNiftyHistory("6mo").catch(() => ({ candles: [], name: "NIFTY 50", currentPrice: null }))
    ),
    fetchNiftyOptionChain(3).catch(() => ({ available: false, reason: "NSE NIFTY chain fetch failed" })),
    fetchNiftyMonthlyChain(2).catch(() => ({ chain: { available: false }, monthlyExpiry: null })),
