...
const { fetchNiftyOptionChain, fetchNiftyMonthlyChain, pickMonthlyExpiry } = require("./nse-options");
...
async function buildInstitutionalStrategyDashboard() {
  ...
  const [history, liveChain, monthlyPack, vix, fiiDii] = await Promise.all([
    fetchNiftyHistory("6mo").catch(() => ({ candles: [], name: "NIFTY 50", currentPrice: null })),
    fetchNiftyOptionChain(2).catch(() => ({ available: false, reason: "NSE NIFTY chain fetch failed" })),
    fetchNiftyMonthlyChain(1).catch(() => ({ chain: { available: false }, monthlyExpiry: null })),
    fetchVix(),
    fetchFiiDiiSafe(),
  ]);
  ...
  const monthlyExpiry =
    monthlyPack.monthlyExpiry || pickMonthlyExpiry(chain?.expiries || monthlyPack.chain?.expiries);
  const monthlyChain = monthlyPack.chain?.available ? monthlyPack.chain : null;
  ...
  if (!candidates.length && !marketStatus.isLive) {
    ...
    candidates = supplementCandidates(
      candidates,
      { ...context, vix: vix?.value, chain, monthlyExpiry },
      "NIFTY",
      10
    ).filter((strategy) => !isExpiredExpiry(strategy.expiry, marketStatus.sessionDate));
  }
  ...
  let top10 = candidates.length
    ...
    : [];
  top10 = top10.filter((strategy) => !isExpiredExpiry(strategy.expiry, marketStatus.sessionDate));

  top10 = prepareDashboardStrategies(
    assignSequentialRanks(finalizeStrategies(top10, marketStatus, chainResolution))
      .map((strategy) => alignExpiryType(strategy, monthlyExpiry))
      .map((strategy) => applyStrategyEligibility(strategy, {
        marketStatus,
        technical: { trend, rsi: latest.rsi, adx: latest.adx, support: latest.support, resistance: latest.resistance },
        assetClass: "index",
      }))
  );
  ...
  return {
    ...
    subtitle: marketStatus.isLive
      ...
      : `Top 10 NIFTY plans for ${marketStatus.planningDateLabel || "the next trading session"} — latest verified market close`,
    ...
    chainStatus: chainResolution.verified
      ? {
          ...
          expiry: chain.expiry,
          monthlyExpiry: monthlyExpiry ?? null,
