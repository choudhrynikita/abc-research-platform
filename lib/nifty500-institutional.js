...
function applyFundamentals(base, fundamentals) {
  ...
  return {
    ...
    // Yahoo ownership summary when present (not NSE promoter/FII/DII categories)
    institutionalHolding: sh.institutional ?? unavailable("institutional", "Yahoo institutional % unavailable; NSE categories require licensed feed"),
    promoterHolding: sh.promoter ?? unavailable("promoter", "Promoter holding requires NSE/BSE shareholding feed"),
    fiiChange: sh.fii ?? unavailable("fiiChange", "FII holding requires NSE shareholding feed"),
    diiChange: sh.dii ?? unavailable("diiChange", "DII holding requires NSE shareholding feed"),
    mutualFundHolding: sh.mutualFunds ?? unavailable("mutualFund", "Mutual fund % not in Yahoo majorHolders for this symbol"),
...
async function buildInstitutionalDashboardFresh() {
  ...
  // Phase 1: full-universe technical screen (bounded concurrency)
  const screened = await mapPool(constituentsMeta, SCREEN_CONCURRENCY, fetchScreenQuote);
  ...
  const payload = {
    ...
    universe: {
      ...
      methodology: {
        phase1: "1y OHLCV technical screen (bounded concurrency)",
