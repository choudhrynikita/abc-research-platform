...
async function fetchScreenQuote(item) {
  try {
    const chart = await fetchChart(item.symbol, "1d", "1y");
...
async function buildInstitutionalDashboard() {
  if (dashboardCache.value && Date.now() < dashboardCache.expiresAt) {
    return {
