...
async function analyzeEquity(symbol, meta, marketTrend, sectorMap) {
  ...
  if (monthlyExpiry) {
    chain = await fetchOptionChain(symbol, 2, monthlyExpiry);
    if (chain.available) {
  ...
  const candidates = generateCandidates(chain, ctx).map((c) => ({ ...c, _ctx: ctx }));
...
function buildInsights(top10, marketContext) {
  ...
  if (marketContext.chainsVerified > 0) {
    ...
    risks.push("No verified NSE equity option chains — strategies suppressed until market data available.");
  }
...
async function buildInstitutionalEquityFnoDashboard() {
  ...
  const analyses = await Promise.all(
    symbols.map((sym) =>
      analyzeEquity(sym, metaMap[sym] || { name: sym.replace(".NS", ""), sector: "—" }, marketTrend, sectorMap)
    )
  );
  ...
  const top10 = rankTop10(allCandidates, globalContext);
  ...
  return {
    ...
    subtitle: "Top 10 highest-conviction equity options strategies — monthly expiry only",
    refreshedAt: new Date().toISOString(),
    source: chainsVerified > 0
      ? "NSE option-chain-equities + Yahoo Finance + ABC quantitative engine"
      : "Yahoo Finance (NSE chains unavailable — limited strategies)",
    executiveSummary: {
      marketTrend,
      ...
      chainsVerified,
      strategiesActive: top10.filter((s) => s.status === "Active").length,
    ...
    backtest: { available: false, note: "Equity options backtest requires verified historical options data — not available" },
