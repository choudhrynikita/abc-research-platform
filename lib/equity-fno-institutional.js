...
const { resolveEquityChain, getEquityChainCache } = require("./option-chain-cache");
const {
  finalizeStrategies,
...
const { summarizeFinancialContext, applyStrategyEligibility, isExpiredExpiry, parseExpiry } = require("./strategy-eligibility");
const { attachPositioning } = require("./strategy-positioning");

const FINANCIAL_REVIEW_TTL_MS = 4 * 60 * 60 * 1000;
const financialReviewCache = new Map();
...
function alignExpiryType(strategy, monthlyExpiry) {
  ...
  if (strategyExpiry && monthly && strategyExpiry.getTime() === monthly.getTime()) {
    return {
      ...
      dossier: strategy.dossier ? { ...strategy.dossier, holdingPeriod: "Monthly expiry" } : strategy.dossier,
    };
  }
  return strategy;
}

async function getFinancialReview(symbol) {
  const cached = financialReviewCache.get(symbol);
  if (cached && Date.now() - cached.cachedAt < FINANCIAL_REVIEW_TTL_MS) return cached.value;
  try {
    const value = summarizeFinancialContext(await fetchFundamentals(symbol));
    financialReviewCache.set(symbol, { value, cachedAt: Date.now() });
    return value;
  } catch {
    const value = summarizeFinancialContext(null);
    financialReviewCache.set(symbol, { value, cachedAt: Date.now() });
    return value;
  }
}
...
async function analyzeEquity(symbol, meta, marketTrend, sectorMap, marketStatus) {
  ...
  let ivMetrics = null;
  if (monthlyExpiry) {
    let liveChain = { available: false };
    // Weekend / holiday: prefer last verified cache so the tab does not wait on 10 NSE calls.
    if (!marketStatus?.isLive) {
      const cached = await getEquityChainCache(nseSymbol);
      if (cached?.strikes?.length) {
        liveChain = { ...cached, available: true };
      }
    }
    if (!liveChain.available) {
      liveChain = await fetchOptionChain(symbol, 1, monthlyExpiry);
    }
  ...
  const ctx = {
    ...
    // Verified equity OHLCV for underlying directional proxy backtests
    candles,
  };

  let candidates = generateCandidates(chain, ctx)
    .filter((strategy) => !isExpiredExpiry(strategy.expiry, marketStatus?.sessionDate))
    .map((c) => ({ ...c, _ctx: ctx }));
...
function buildInsights(top10, marketContext) {
  ...
  if (marketContext.chainsVerified > 0) {
    ...
    options.push("Planning setups generated from verified price and indicator data.");
...
function supplementEquityCandidates(analyses, target = 10) {
  const merged = analyses.flatMap((a) => a.candidates);
  if (merged.length >= target) return merged;

  const seen = new Set(merged.map((s) => `${s.nseSymbol || s.symbol}|${s.name}`));
  for (const analysis of analyses) {
    ...
    const ctx = {
      ...
      support: analysis.technicals?.support ?? analysis.marketContext?.support,
      resistance: analysis.technicals?.resistance ?? analysis.marketContext?.resistance,
      rsi: analysis.technicals?.rsi,
      adx: analysis.technicals?.adx,
      volumeTrend: analysis.technicals?.volumeTrend,
      chain: analysis.chain,
      monthlyExpiry: analysis.monthlyExpiry,
      relativeStrength: analysis.marketContext?.relativeStrength,
      sectorOutlook: analysis.marketContext?.sectorTrend,
      marketTrend: analysis.marketContext?.marketTrend,
    };
    const extras = generateTechnicalSetups(ctx, analysis.nseSymbol);
    for (const setup of extras) {
      if (merged.length >= target) break;
      const key = `${setup.nseSymbol || analysis.nseSymbol}|${setup.name}`;
      if (seen.has(key)) continue;
      seen.add(key);
      if (!isExpiredExpiry(setup.expiry, analysis.marketContext?.sessionDate)) {
        merged.push({ ...setup, _ctx: analysis.candidates[0]?._ctx || ctx });
      }
    }
  }
  return merged;
...
async function buildInstitutionalEquityFnoDashboard() {
  ...
  const [niftyChart, vix, fiiDii] = await Promise.all([
    fetchChart("^NSEI", "1d", "6mo").catch(() => ({ candles: [], meta: {} })),
    fetchVix(),
  ...
  const niftyIndicators = niftyCandles.length ? computeIndicators(niftyCandles) : { latest: {} };
  ...
  let allCandidates = analyses.flatMap((a) => a.candidates);
  if (allCandidates.length < 10) {
    allCandidates = supplementEquityCandidates(analyses, 10);
  }
  ...
  let top10 = allCandidates.length
    ? (!marketStatus.isLive && chainsVerified === 0
      ? rankPreMarketSetups(allCandidates, globalContext)
      : rankTop10(allCandidates, globalContext))
    : [];

  const chainMeta = {
    stale: chainsStale > 0 || !marketStatus.isLive,
    fetchedAt: marketStatus.checkedAt,
  };
  top10 = assignSequentialRanks(finalizeStrategies(top10, marketStatus, chainMeta));
  const financialEntries = await Promise.all(
    [...new Set(top10.map((strategy) => strategy.symbol).filter(Boolean))].map(async (symbol) => {
      return [symbol, await getFinancialReview(symbol)];
    })
  );
  const financialBySymbol = Object.fromEntries(financialEntries);
  top10 = prepareDashboardStrategies(top10.map((strategy) => {
    const analysis = analyses.find((entry) => entry.symbol === strategy.symbol);
    const eligible = applyStrategyEligibility(alignExpiryType(strategy, strategy._ctx?.monthlyExpiry), {
      marketStatus,
      technical: {
        trend: analysis?.trend,
        rsi: analysis?.technicals?.rsi,
      ...
      financial: financialBySymbol[strategy.symbol],
      assetClass: "equity",
    });
    return attachPositioning(eligible, analysis?.chain);
  }));
  ...
  return {
    ...
    subtitle: marketStatus.isLive
      ...
      : `Top 10 equity plans for ${marketStatus.planningDateLabel || "the next trading session"} — latest verified market close`,
    ...
    executiveSummary: {
      ...
      chainsVerified,
      chainsStale,
      strategiesTotal: top10.length,
      strategiesActive: top10.filter((s) => s.eligibility?.decision === "LIVE" || s.eligibility?.decision === "PLAN").length,
