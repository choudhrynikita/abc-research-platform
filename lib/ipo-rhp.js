...
async function extractDossierFromPdf(pdfBuf) {
  ...
  const financialText = joinPages(pages, (p) => (
    !looksLikeToc(p.text)
    && /as Restated|RESTATED STATEMENT|TOTAL\(A\+B\)|Total assets|FINANCIAL KPIs|Revenue from Operations/i.test(p.text)
  ));
  const objectsText = joinPages(pages, (p) => (
    !looksLikeToc(p.text)
    && /OBJECT(?:S)? OF THE (OFFER|ISSUE)|Requirement of [Ff]unds|Utili[sz]ation of Net Proceeds|intend to utili[sz]e the Net Proceeds|proposes to utili[sz]e the Net Proceeds|Estimated Amount[\s\S]{0,24}\(₹\s*million\)|Funding inorganic growth through unidentified|Capital expenditure of our Company through purchase of equipment/i.test(p.text)
  ));
  const businessText = joinPages(pages, (p) => (
    !looksLikeToc(p.text)
    && /OUR BUSINESS|BUSINESS OVERVIEW|OUR COMPETITIVE STRENGTHS|Our Strengths|Overview\s+We are|Our Company is an |We operate as|We are one of|Strategically located/i.test(p.text)
  ));
