...
const { fetchRhpArchive, nseRhpArchiveUrl } = require("./nse-ipo");
...
function bundledProspectus(symbol) {
  const s = String(symbol || "").toUpperCase();
  try {
    if (s === "KHERIAAUTO") return require("../data/ipo-prospectus/KHERIAAUTO.json");
    if (s === "HEROMOTORS") return require("../data/ipo-prospectus/HEROMOTORS.json");
    if (s === "SPECTRAA") return require("../data/ipo-prospectus/SPECTRAA.json");
  } catch {
    return null;
  }
  return null;
...
async function extractDossierFromPdf(pdfBuf) {
  ...
  const finP = tocPrintedPage(toc, "SUMMARY OF FINANCIAL INFORMATION")
    || tocPrintedPage(toc, "SUMMARY OF RESTATED CONSOLIDATED FINANCIAL STATEMENTS");
  ...
  const indP = tocPrintedPage(toc, "INDUSTRY OVERVIEW");
  ...
  for (const n of rangeAround(bizP, offset, 0, 22, 800)) wanted.add(n);
  for (const n of rangeAround(indP, offset, 0, 12, 800)) wanted.add(n);
  ...
  const financialText = joinPages(pages, (p) => (
    !looksLikeToc(p.text)
    && /as Restated|RESTATED STATEMENT|SUMMARY OF RESTATED|TOTAL\(A\+B\)|Total assets|FINANCIAL KPIs|Revenue from Operations|Amount in INR lakhs|Profit for the period/i.test(p.text)
  ));
  const objectsText = joinPages(pages, (p) => (
    !looksLikeToc(p.text)
    && /OBJECT(?:S)? OF THE (OFFER|ISSUE)|Requirement of [Ff]unds|Utili[sz]ation of Net Proceeds|intend to utili[sz]e the Net Proceeds|proposes to utili[sz]e the Net Proceeds|Estimated Amount[\s\S]{0,24}\(₹\s*million\)|Funding inorganic growth through unidentified|Capital expenditure of our Company through purchase of equipment|Capital Expenditure at Jaipur/i.test(p.text)
  ));
  const businessText = joinPages(pages, (p) => (
    !looksLikeToc(p.text)
    && /OUR BUSINESS|BUSINESS OVERVIEW|OUR COMPETITIVE STRENGTHS|Our Strengths|\bSTRENGTHS\b|Overview\s+We are|Our Company is an |We operate as|We are one of|Strategically located|We provide engineering|Our Company, SpectraA|two manufacturing facilities located|We operate in a fragmented/i.test(p.text)
  ));
  const industryText = joinPages(pages, (p) => (
    !looksLikeToc(p.text)
    && /INDUSTRY OVERVIEW|India Beer and Malt|Credence Report|fragmented and highly competitive/i.test(p.text)
  ));
  ...
  return parseRhptSections({ coverText, riskText, financialText, objectsText, businessText, industryText });
}
...
async function loadProspectusDossier({ symbol, rhpUrl } = {}) {
  ...
  const bundled = bundledProspectus(symbol);
  if (bundled?.available) {
    return { ...bundled, symbol, rhpUrl: url || bundled.rhpUrl, bundled: true };
  }
  ...
  let lastError = null;
  try {
    const zipBuf = await fetchRhpArchive(url, 90_000);
    ...
    if (payload.available) {
      await setDashboardCache(key, payload);
      return payload;
    }
  } catch (err) {
    lastError = err;
  ...
  if (bundled?.available) {
    const payload = {
      ...
      rhpUrl: url,
      bundled: true,
      liveParseError: lastError?.message || null,
    };
    await setDashboardCache(key, payload);
    return payload;
  }
...
module.exports = { loadProspectusDossier, parseRhptSections, bundledProspectus, extractDossierFromPdf };
