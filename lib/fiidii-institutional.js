...
const LIVE_FII_DII_BUDGET_MS = 6_000;
let liveRequestInFlight = null;

function withinBudget(promise, budgetMs = LIVE_FII_DII_BUDGET_MS) {
  return Promise.race([
    promise,
    new Promise((_, reject) => setTimeout(() => reject(new Error("Live NSE FII/DII request timed out")), budgetMs)),
  ]);
}

function getLiveFiiDiiRequest() {
  if (!liveRequestInFlight) {
    liveRequestInFlight = fetchFiiDii(1).finally(() => {
...
async function fetchLiveWithRetry() {
  let lastError;
  try {
    const data = await withinBudget(getLiveFiiDiiRequest());
