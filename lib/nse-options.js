...
function analyzeChain(data, sourceLabel = "NSE India option-chain API", selectedExpiry = null) {
  ...
  if (!rows.length) return { available: false, reason: "No strikes for selected expiry" };
...
function parseNseExpiry(expiryStr) {
  ...
  let parsed = new Date(expiryStr);
  if (!Number.isNaN(parsed.getTime())) return parsed;
  const numeric = String(expiryStr).match(/^(\d{1,2})-(\d{2})-(\d{4})$/);
  if (numeric) {
    parsed = new Date(Number(numeric[3]), Number(numeric[2]) - 1, Number(numeric[1]));
...
function pickMonthlyExpiry(expiries) {
  ...
  const target = nextMonthlyExpiry();
