...
/**
 * Y-axis bounds from verified candle high/low (plus optional overlay points).
 * Chart.js LinearScale only reads parsed.y; candlesticks expose o/h/l/c, so mixed
 * SMA overlays would otherwise collapse the scale to the moving-average range and
 * let wicks paint outside the plot (and over the next dashboard section).
 * Never invents prices — returns null when no verified highs/lows exist.
 */
function priceScaleBounds(candles = [], extraPoints = []) {
  let min = Infinity;
  let max = -Infinity;
  ...
  for (const candle of candles) {
    ...
    const values = [
      Number(candle.high ?? candle.h),
      Number(candle.low ?? candle.l),
      Number(candle.open ?? candle.o),
      Number(candle.close ?? candle.c),
    ];
  ...
  for (const point of extraPoints) {
    const y = typeof point === "number" ? point : Number(point?.y);
  ...
  const span = max - min;
  const pad = span > 0 ? span * 0.05 : max * 0.01;
  return {
