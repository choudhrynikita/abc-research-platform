...
const PROSPECTUS_NOT_PUBLISHED = "Not supplied by NSE IPO feed; requires verified DRHP/RHP filings.";
...
function buildScorecard(ipo, subscription, listedEnriched) {
  const dimensions = [
    { key: "businessQuality", label: "Business Quality", available: false, score: null, note: PROSPECTUS_NOT_PUBLISHED },
    { key: "financialHealth", label: "Financial Health", available: false, score: null, note: PROSPECTUS_NOT_PUBLISHED },
    { key: "profitability", label: "Profitability", available: false, score: null, note: PROSPECTUS_NOT_PUBLISHED },
    { key: "growth", label: "Growth", available: false, score: null, note: PROSPECTUS_NOT_PUBLISHED },
    { key: "valuation", label: "Valuation", available: false, score: null, note: PROSPECTUS_NOT_PUBLISHED },
    { key: "management", label: "Management Quality", available: false, score: null, note: PROSPECTUS_NOT_PUBLISHED },
    { key: "industry", label: "Industry Outlook", available: !!ipo.industry, score: ipo.industry ? 50 : null, note: ipo.industry ? `Industry: ${ipo.industry} (NSE tag only)` : NSE_NOT_PUBLISHED },
    { key: "risk", label: "Risk Level", available: subscription?.overall?.available, score: subscription?.overall?.value != null ? (subscription.overall.value < 1 ? 75 : 45) : null, note: subscription?.overall?.available ? "Based on verified subscription" : NSE_NOT_PUBLISHED },
    { key: "listingGain", label: "Listing Gain Potential", available: subscription?.overall?.available, score: subscription?.overall?.value != null ? Math.min(100, subscription.overall.value * 22) : null, note: subscription?.overall?.display || NSE_NOT_PUBLISHED },
    { key: "longTerm", label: "Long-Term Potential", available: false, score: null, note: PROSPECTUS_NOT_PUBLISHED },
  ];
...
function buildIpoCard(ipo, extras = {}) {
  ...
  return {
    ...
    logo: null,
    industry: ipo.industry || NSE_NOT_PUBLISHED,
    sector: NSE_NOT_PUBLISHED,
    ipoType: ipoType(ipo),
    issueSize: ipo.issueSize || NSE_NOT_PUBLISHED,
    priceBand: ipo.issuePrice || NSE_NOT_PUBLISHED,
    lotSize: ipo.lotSize ?? NSE_NOT_PUBLISHED,
    minInvestment: minInv ?? NSE_NOT_PUBLISHED,
    exchange: ipo.exchange || "NSE",
    openDate: ipo.issueStartDate || null,
    closeDate: ipo.issueEndDate || null,
    allotmentDate: NSE_NOT_PUBLISHED,
    ...
    subscription: subscription
      ...
      : ipo.overallSubscription != null
        ...
        : { overall: { available: false, value: null, display: NSE_NOT_PUBLISHED, source: "NSE" } },
...
async function buildInstitutionalIpoDetail(symbol) {
  ...
  return {
    ...
    recommendation,
    scorecard,
    subscription: subscription || { overall: { available: false, display: NSE_NOT_PUBLISHED, source: "NSE" } },
    fundamentals: {
      available: false,
      message: PROSPECTUS_NOT_PUBLISHED,
      metrics: {
        revenueTrend: null,
        profitTrend: null,
        ebitdaTrend: null,
      ...
      bullets: [
        ...
        PROSPECTUS_NOT_PUBLISHED,
      ],
    },
    financialCharts: { available: false, message: PROSPECTUS_NOT_PUBLISHED, series: null },
    valuation: {
      available: false,
    ...
    peers: { available: false, message: PROSPECTUS_NOT_PUBLISHED, peers: [], highlights: null },
    industryOutlook: {
      available: !!ipo.industry,
      industry: ipo.industry,
      outlook: ipo.industry ? "Neutral" : null,
      bullets: ipo.industry
        ? [`NSE industry classification: ${ipo.industry}`, "Detailed industry growth statistics require licensed research feed", PROSPECTUS_NOT_PUBLISHED]
        : [NSE_NOT_PUBLISHED],
    },
    risks: {
      bullets: [
