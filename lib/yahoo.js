...
async function fetchChartFrom(host, symbol, interval, range) {
...
async function fetchChart(symbol, interval = "1d", range = "1y") {
  ...
  for (const host of YAHOO_HOSTS) {
    try {
      return await fetchChartFrom(host, symbol, interval, range);
    } catch (err) {
      lastErr = err;
    }
  }
  throw lastErr || new Error(`No chart data for ${symbol}`);
}
