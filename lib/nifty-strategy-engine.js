...
function generateCandidates(chain, context) {
  ...
  // 10. Straddle watch — breakout preparation (no premium until triggered)
  if (atm != null && support != null && resistance != null) {
    candidates.push(makeStrategy({
      name: "ATM Straddle Watch",
      type: "Straddle Watch",
      bias: "Neutral",
      expiryType: "Weekly",
      expiry,
      ...
      status: context.trend === "NEUTRAL" ? "Active" : "Wait",
      entryTrigger: `Wait for decisive break of ${support}–${resistance} range`,
    }, context));
  }
