...
function attachOptionsDossier(strategy, context) {
  ...
  // Prefer underlying directional proxy when verified OHLCV is available.
  // Never invent multi-leg premium P&L — proxy is explicitly labeled.
  const candles = context?.candles;
  let backtest;
  if (Array.isArray(candles) && candles.length >= 80 && strategy.bias && strategy.bias !== "Neutral") {
    backtest = backtestDirectionalProxy(candles, strategy.bias);
  } else {
    backtest = {
      available: false,
      reason: Array.isArray(candles) && candles.length < 80
        ? "Awaiting Latest Verified Data — need ≥80 underlying daily bars for directional proxy backtest"
        : strategy.bias === "Neutral"
          ? "Underlying directional proxy not applicable for Neutral structures; multi-leg premium history unavailable"
          : "Backtest could not be completed — no verified underlying OHLCV attached; multi-leg option premiums unavailable from free feeds",
      samples: 0,
      proxyType: "none",
      assumptions: [
        "Full multi-leg premium backtest requires historical NSE option series (not in free feeds)",
        "When underlying history is present, a directional proxy may be used and is clearly labeled",
        "Never invent win rates or success percentages",
      ],
    };
  }
