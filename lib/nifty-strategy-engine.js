...
const { applyHonestManagementTargets } = require("./strategy-payload");
const { formatTradeBadge, formatTradeLine } = require("./strategy-trade-label");
const { rateLiquidityFromLegs } = require("./strategy-eligibility");
...
function rankingRiskReward(strategy) {
  if (strategy.payoff?.maxProfitUnlimited) return null;
  if (isNakedLong(strategy.type)) return null;
  return strategy.riskRewardRatio ?? strategy.payoff?.riskRewardRatio ?? null;
}

function shouldOfferBias(ctx, bias) {
  const trend = ctx?.trend || ctx?.marketTrend;
  const rsi = ctx?.rsi;
  if (bias === "Bullish") {
    if (trend === "BEARISH") return false;
    if (trend === "NEUTRAL" && rsi != null && rsi < 48) return false;
    return true;
  }
  if (bias === "Bearish") {
    if (trend === "BULLISH") return false;
    if (trend === "NEUTRAL" && rsi != null && rsi > 52) return false;
    return true;
  }
  return true;
}
...
function scoreStrategy(strategy, context) {
  ...
  } else if (context.trend === "BEARISH" && strategy.bias === "Bearish") {
    score += 12;
    factors.push("Trend alignment");
  } else if (context.trend === "NEUTRAL" && strategy.bias === "Neutral") {
    score += 14;
    factors.push("Neutral trend fit");
  } else if (context.trend === "BULLISH" && strategy.bias === "Bearish") {
    score -= 20;
    factors.push("Counter-trend (penalized)");
  } else if (context.trend === "BEARISH" && strategy.bias === "Bullish") {
    score -= 20;
    factors.push("Counter-trend (penalized)");
  } else if (context.trend === "NEUTRAL" && (strategy.bias === "Bullish" || strategy.bias === "Bearish")) {
    score -= 8;
    factors.push("Directional idea in a range-bound tape");
  }
  ...
  const rr = rankingRiskReward(strategy);
  if (rr != null && rr >= 1.5) {
    score += 8;
    factors.push("Favorable defined-risk reward/risk");
  } else if (rr != null && rr < 0.4) {
    score -= 10;
    factors.push("Poor defined-risk reward/risk");
  }
...
function hydrateStrikes(strikes, chain) {
  if (!Array.isArray(strikes)) return [];
  return strikes.map((leg) => {
    if (!leg) return leg;
    const live = chain && leg.strike != null && leg.type
      ? getLegAtStrike(chain, leg.strike, leg.type)
      : null;
    return {
      ...leg,
      openInterest: leg.openInterest ?? live?.openInterest ?? null,
      volume: leg.volume ?? live?.volume ?? null,
    };
  });
}

function attachIndexAnalytics(strategy) {
  const legs = Array.isArray(strategy.strikes) ? strategy.strikes : [];
  const oi = Math.max(0, ...legs.map((leg) => Number(leg?.openInterest) || 0));
  const volume = Math.max(0, ...legs.map((leg) => Number(leg?.volume) || 0));
  return {
    ...strategy,
    analytics: {
      ...(strategy.analytics || {}),
      liquidityRating: rateLiquidityFromLegs(legs),
...
function makeStrategy(base, ctx) {
  ...
  const strikes = hydrateStrikes(base.strikes, ctx?.chain);
  ...
  let enriched = {
    ...base,
    strikes,
    lotSize: lotSize ?? base.lotSize ?? null,
    lastUpdated: new Date().toISOString(),
  };

  const hasTradeableLegs =
    Array.isArray(strikes) &&
    strikes.some((s) => s && s.premium != null && (s.action === "BUY" || s.action === "SELL"));

  if (hasTradeableLegs) {
    enriched = enrichStrategyWithPayoff(enriched, { spot, lotSize });
    enriched = applyHonestManagementTargets(enriched);
  } else {
...
function generateMonthlyStrategies(monthlyChain, monthlyExpiry, context) {
  ...
  if (monthlyCe?.premium != null && shouldOfferBias(context, "Bullish") && context.trend === "BULLISH") {
  ...
  if (monthlyPe?.premium != null && shouldOfferBias(context, "Bearish") && context.trend === "BEARISH") {
  ...
  if (otmCallStrike && otmCallStrike !== atm && shouldOfferBias(context, "Bullish")) {
...
function generateCandidates(chain, context, options = {}) {
  const candidates = [];
  const ctx = { ...context, chain: context?.chain || chain };
  ...
  if (longCeLegs && shouldOfferBias(context, "Bullish") && context.trend === "BULLISH") {
  ...
  if (longPeLegs && shouldOfferBias(context, "Bearish") && context.trend === "BEARISH") {
  ...
  // 4. Bear Put Spread
  if (otmPutStrike && otmPutStrike !== atm && shouldOfferBias(context, "Bearish")) {
  ...
  if (
    ...
    shouldOfferBias(context, "Bullish")
  ) {
  ...
  if (
    ...
    shouldOfferBias(context, "Bearish")
  ) {
  ...
  if (otmCallStrike && otmCallStrike !== atm) {
    ...
    if (leg?.premium != null && shouldOfferBias(context, "Bullish") && context.trend === "BULLISH") {
  ...
  if (otmPutStrike && otmPutStrike !== atm) {
    ...
    if (leg?.premium != null && shouldOfferBias(context, "Bearish") && context.trend === "BEARISH") {
      const prem = leg.premium;
      candidates.push(makeStrategy({
        name: "Long OTM Put",
        type: "Long PE",
  ...
  // 9. Protective Put — portfolio hedge (only when tape is actually bearish)
  if (otmPutStrike) {
    const leg = getLegAtStrike(chain, otmPutStrike, "PE");
    if (leg?.premium != null && shouldOfferBias(context, "Bearish") && context.trend === "BEARISH") {
      const prem = leg.premium;
  ...
  const includeMonthly = options.includeMonthly ?? (!options.horizon && Boolean(monthlyChain?.available && monthlyExpiry));
  if (includeMonthly) {
    candidates.push(...generateMonthlyStrategies(monthlyChain, monthlyExpiry, context));
  }

  if (options.horizon) {
    return applyHorizon(candidates, options.horizon);
  }
  return candidates;
}
...
function applyHorizon(strategies, horizon) {
  if (!horizon || !Array.isArray(strategies)) return strategies || [];
  const prefix = horizon.id === "monthly" ? "Monthly " : horizon.id === "15-day" ? "15-Day " : "7-Day ";
  return strategies.map((strategy) => {
    const next = {
      ...strategy,
      horizon: horizon.id,
      horizonLabel: horizon.label || horizon.id,
      daysToExpiry: horizon.daysAway ?? strategy.daysToExpiry ?? null,
      expiryType: horizon.expiryType || strategy.expiryType,
      expiry: horizon.expiry || strategy.expiry,
      holdingPeriod: horizon.holdingPeriod || strategy.holdingPeriod,
      name: String(strategy.name || "").startsWith(prefix) ? strategy.name : `${prefix}${strategy.name}`,
    };
    const tradeLine = formatTradeLine(next);
    return {
      ...next,
      tradeLabel: formatTradeBadge(next),
      tradeLine,
      modeLabel: tradeLine || strategy.modeLabel,
    };
  });
}

function rankTop10(candidates, context, limit = 10) {
  // Rank on cheap structural scores first. Synthetic backtests on every
  // candidate were timing out the Vercel function and blanking both tabs.
  ...
  return ranked.map((s, i) => {
    const attached = attachOptionsDossier(s, context);
    return {
      ...s,
