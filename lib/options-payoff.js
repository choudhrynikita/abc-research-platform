...
function classifyBounds(legs, prices) {
  ...
  // Long naked call → unlimited profit; short naked call → unlimited loss (as S→∞)
  const maxProfitUnlimited = callExposure > 0;
  // Only net short CALL exposure produces theoretically unlimited loss.
  // Short puts have finite max loss ≈ strike − premium as S→0 (never "unlimited").
  const maxLossUnlimited = callExposure < 0;
  // putExposure is retained for documentation / far-low grid evaluation only
  void putExposure;
