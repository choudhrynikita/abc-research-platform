...
function macd(closes, fast = 12, slow = 26, signalPeriod = 9) {
  ...
  // Pass nulls through — ema() already skips nulls and never seeds on invented zeros.
  // Do NOT coerce null → 0 (that fabricated early MACD history and biased the signal).
...
function findSupportResistance(candles, lookback = 20) {
  ...
  return {
    support: lows.length ? Number(Math.min(...lows).toFixed(2)) : null,
    resistance: highs.length ? Number(Math.max(...highs).toFixed(2)) : null,
  };
}
