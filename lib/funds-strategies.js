...
function premiumLabel(p) {
  ...
  if (Math.abs(p) > 5) {
    return `Do not fill — last vs AMFI NAV disagree by ${p.toFixed(2)}%. Confirm iNAV on NSE; this is not a bargain`;
  }
...
function fundSheet(plan) {
  ...
  const qty = sip && plan.lots
    ...
    : plan.lots != null
      ? (Number(plan.lots) === 0 ? "0 units — do not send" : `${plan.lots} units (${plan.lotSpec || "CNC"})`)
      : plan.lotSpec;
  return {
...
function buildFundDeskPlans({ etfs = [], featured = [], navDate = null } = {}) {
  ...
  if (junior?.price && bees?.price) {
    ...
    push(out, {
      ...
      contract: "JUNIORBEES",
      lotSpec: "NSE cash, CNC",
      lots: unitsFor(3000, junior.price),
      last: junior.price,
  ...
  if (mid?.price) {
    push(out, {
      ...
      contract: "MID150BEES",
      lotSpec: "NSE cash, CNC",
      lots: mid.ret3m != null && mid.ret3m > 18 ? 0 : unitsFor(2000, mid.price),
      last: mid.price,
  ...
  if (silver?.price) {
    push(out, {
      ...
      contract: "SILVERBEES",
      lotSpec: "NSE cash, CNC",
      lots: unitsFor(2000, silver.price),
      last: silver.price,
  ...
  if (indexFund && bees?.price) {
    ...
    push(out, {
      ...
      lots: beesRich ? 10000 : unitsFor(10000, bees.price),
      entryZone: null,
  ...
  if (nasdaq?.price) {
    const skip = nasdaq.premiumPct != null && nasdaq.premiumPct > 0.7;
    const units = unitsFor(5000, nasdaq.price);
    push(out, {
      id: "nasdaq-cap",
      name: skip
        ? "Skip Nasdaq 100 ETF this print"
        : "Nasdaq 100 ETF — cap at 10%, remember the tax wrapper",
      ...
      contract: nasdaq.nse,
      contractName: nasdaq.name,
      lotSpec: "NSE cash, CNC",
      lots: skip ? 0 : units,
      last: nasdaq.price,
      tradeLine: skip
        ? `WAIT ${nasdaq.nse} · ₹${nasdaq.price} · ${premiumLabel(nasdaq.premiumPct)}`
        : `BUY ${nasdaq.nse} · ₹${nasdaq.price} · keep ≤ 10% of the book · ${premiumLabel(nasdaq.premiumPct)}`,
      entryZone: { low: round(nasdaq.price, 2), high: round(nasdaq.price, 2) },
      stopLoss: skip
        ? "Do not send until last and iNAV agree."
        : "Hard cap 10%. This is US tech beta in an Indian ETF, not 'diversification magic'.",
      targets: { t1: "5–10% satellite", t2: null },
      tradeTicket: {
        action: skip ? "WAIT" : "BUY",
        steps: skip
          ? [
              `Open NSE. Search ${nasdaq.nse}. Last ₹${nasdaq.price} vs NAV ₹${nasdaq.nav ?? "—"}.`,
              premiumLabel(nasdaq.premiumPct) + " — do not click BUY today.",
              "A fat last-vs-NAV gap on an international ETF is usually iNAV lag or a stale print, not a bargain.",
            ]
          : [
              `BUY ${nasdaq.nse} CNC at ₹${nasdaq.price}. Tracks Nasdaq 100.`,
              premiumLabel(nasdaq.premiumPct),
      ...
      invalidation: skip
        ? "No fill while last and AMFI NAV disagree. Confirm iNAV on NSE first."
        : "No add if already > 10%, or if you need rupee cash inside 2 years.",
    });
  }
  ...
  if (liquid?.price || liqFund) {
    ...
    push(out, {
      ...
      lotSpec: liqFund ? "AMC / MF app" : "NSE cash, CNC",
      lots: liqFund ? 10000 : unitsFor(10000, liquid?.price),
      tradeLine: liqFund
