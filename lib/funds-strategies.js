  ...
  if (bees?.price) {
    ...
    push(out, {
      ...
      tradeTicket: {
        ...
        steps: skip
          ? [
              `NIFTYBEES last ₹${bees.price} vs AMFI NAV ₹${bees.nav ?? "—"} (${premiumLabel(bees.premiumPct)}).`,
              "Do not fill a lump or a SIP today. Authorised participants usually compress a fat premium — wait.",
              "If you still want Nifty 50 today, use a Nifty index fund (NAV, not a live premium).",
            ]
          : [
              `BUY NIFTYBEES on NSE, CNC (delivery). Not MIS, not MTF, not F&O.`,
              `Size: ₹10,000 SIP → about ${sipUnits} units at ₹${bees.price}. First trading day of the month.`,
              premiumLabel(bees.premiumPct) + (bees.nav != null ? ` NAV ₹${bees.nav} dated ${bees.navDate || navDate || "—"}.` : ""),
              dip
                ? `1-month return is ${bees.ret1m}%. Add an extra ₹5,000 (≈ ${unitsFor(5000, bees.price)} units) as a dip add — still CNC.`
                : "No extra lump. Let the SIP do the buying.",
              "Horizon 5 years+. You are buying Nifty 50, not trying to beat it.",
            ],
      },
      why: [
