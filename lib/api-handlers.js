...
const { sanitizeCandles, normalizeChartRange, resolveChartRequest, buildChartMeta } = require("./chart-series");
...
const {
  ...
  getPolicyMeta,
  classifyDataType,
} = require("./financial-intelligence");
...
const {
  answerCopilotQuery,
  MAX_QUERY_LEN: MAX_COPILOT_QUERY_LEN,
  SUGGESTED_QUERIES,
  getCopilotStatus,
...
const { getBrokerConfig, fetchBrokerHoldings } = require("./broker-adapter");
...
const API_ROUTES = [
  ...
  { method: "GET", path: "/api/financial-intelligence-policy", description: "Canonical zero-hallucination policy and system prompt" },
  { method: "POST", path: "/api/copilot", description: "AI Research Copilot (public, verified data)" },
  { method: "GET", path: "/api/copilot/suggestions", description: "Copilot suggested queries" },
  { method: "GET", path: "/api/copilot/status", description: "Copilot engine readiness (no secrets exposed)" },
  { method: "POST", path: "/api/strategy-assistant", description: "Context-aware strategy derivatives assistant (public)" },
  { method: "GET", path: "/api/watchlists", description: "Watchlist dashboard with verified live quotes" },
  { method: "POST", path: "/api/watchlists", description: "Create watchlist" },
  { method: "POST", path: "/api/watchlists/:id/symbols", description: "Add symbol to watchlist" },
  { method: "DELETE", path: "/api/watchlists/:id/symbols/:symbol", description: "Remove symbol from watchlist" },
  { method: "GET", path: "/api/portfolios", description: "Portfolio analysis with verified live P&L" },
  { method: "POST", path: "/api/portfolios", description: "Create portfolio" },
  { method: "POST", path: "/api/portfolios/:id/holdings", description: "Upsert portfolio holding" },
  { method: "DELETE", path: "/api/portfolios/:id/holdings/:symbol", description: "Remove portfolio holding" },
  { method: "GET", path: "/api/portfolios/export", description: "Export portfolio holdings CSV" },
  { method: "GET", path: "/api/portfolios/export-analysis", description: "Export portfolio analysis CSV with live P&L" },
  { method: "POST", path: "/api/portfolios/:id/import", description: "Import holdings CSV (cost basis only)" },
  { method: "GET", path: "/api/watchlists/export", description: "Export watchlist symbols CSV" },
  { method: "GET", path: "/api/persistence", description: "Storage mode and broker-link status" },
  { method: "GET", path: "/api/broker/status", description: "Optional broker (Kite) configuration status" },
  { method: "POST", path: "/api/broker/sync-holdings", description: "Import holdings from configured broker (Kite)" },
  { method: "GET", path: "/api/shareholding/:symbol", description: "NSE verified shareholding pattern" },
];
...
async function handleApi({ method, pathname, query = {}, body = null, authHeader = null }) {
  try {
    ...
    if (method === "GET" && pathname === "/api/health") {
      return json(200, {
        ...
        copilot: {
          ...
          llmOptional: true,
          note: "Works without XAI_API_KEY using structured verified-data engines",
        },
    ...
    if (method === "GET" && pathname === "/api/financial-intelligence-policy") {
      return json(200, wrapResponse({
        ...
        source: "ABC Financial Intelligence Policy",
        dataType: DATA_CLASSIFICATION.VERIFIED_FACT,
      }));
    }
    ...
    if (method === "GET" && pathname === "/api/nifty500/top50") {
      ...
      const data = await buildInstitutionalDashboard({ forceRefresh });
    ...
    if (method === "GET" && pathname === "/api/persistence") {
      return json(200, wrapResponse({
        ...getPersistenceInfo(),
        broker: getBrokerConfig(),
      }, {
        source: "ABC storage config",
        dataType: "factual",
        confidence: 100,
      }));
    }

    if (method === "GET" && pathname === "/api/broker/status") {
      return json(200, wrapResponse(getBrokerConfig(), {
        source: "ABC broker adapter",
        dataType: "factual",
        confidence: 100,
      }));
    }

    if (method === "POST" && pathname === "/api/broker/sync-holdings") {
      const portfolioId = body?.portfolioId || query.portfolioId || "default";
      const broker = await fetchBrokerHoldings();
      if (!broker.ok) {
        return json(400, {
          error: broker.error,
          config: broker.config,
          hint: "Configure KITE_API_KEY + KITE_ACCESS_TOKEN or use CSV import",
        });
      }
      let imported = 0;
      for (const h of broker.holdings) {
        const result = await upsertHolding(String(portfolioId), h);
        if (result.ok) imported += 1;
      }
      const analysis = await getPortfolioAnalysis(String(portfolioId));
      return json(200, wrapResponse({
        imported,
        totalFromBroker: broker.count,
        source: broker.source,
        analysis,
      }, {
        source: broker.source || "Broker adapter",
        dataType: "mixed",
        confidence: 90,
        disclaimer: broker.disclaimer,
      }));
    }

    const shMatch = pathname.match(/^\/api\/shareholding\/(.+)$/);
    if (method === "GET" && shMatch) {
      const { fetchShareholding } = require("./shareholding");
      const symbol = decodeURIComponent(shMatch[1]);
      const data = await fetchShareholding(symbol);
      return json(200, wrapResponse(data, {
        source: data.source || "NSE Corporate Filings SHP",
        dataType: data.available ? "factual" : "unavailable",
        confidence: data.available ? 92 : 0,
      }));
    }

    // ── Watchlists (verified quotes only) ──
    if (method === "GET" && pathname === "/api/watchlists/export") {
      const listId = query.id || query.listId || "default";
      const result = await exportWatchlistCsv(String(listId));
      if (result.error) return json(404, { error: result.error });
      return text(200, result.csv, {
    ...
    if (method === "GET" && pathname === "/api/watchlists") {
      const listId = query.id || query.listId || "default";
      const data = await getWatchlistDashboard(String(listId));
      return json(200, wrapResponse({
        ...data,
        persistence: getPersistenceInfo(),
      }, {
        source: "Yahoo Finance Chart API + user watchlist store",
        dataType: "factual",
        confidence: data.available ? 90 : 0,
      }));
    }

    if (method === "POST" && pathname === "/api/watchlists") {
      const list = await createWatchlist(body?.name);
      return json(201, wrapResponse({ list }, { source: "ABC Watchlist Store", dataType: "factual", confidence: 100 }));
    }

    const wlAddMatch = pathname.match(/^\/api\/watchlists\/([^/]+)\/symbols$/);
    if (method === "POST" && wlAddMatch) {
      const result = await addWatchlistSymbol(decodeURIComponent(wlAddMatch[1]), body?.symbol);
      if (result.error) return json(400, { error: result.error });
      return json(200, wrapResponse(result, { source: "ABC Watchlist Store", dataType: "factual", confidence: 100 }));
    }

    const wlDelMatch = pathname.match(/^\/api\/watchlists\/([^/]+)\/symbols\/([^/]+)$/);
    if (method === "DELETE" && wlDelMatch) {
      const result = await removeWatchlistSymbol(
        decodeURIComponent(wlDelMatch[1]),
        decodeURIComponent(wlDelMatch[2])
      );
      if (result.error) return json(404, { error: result.error });
      return json(200, wrapResponse(result, { source: "ABC Watchlist Store", dataType: "factual", confidence: 100 }));
    }

    // ── Portfolios (verified prices × user cost basis) ──
    if (method === "GET" && pathname === "/api/portfolios/export") {
      const portfolioId = query.id || query.portfolioId || "default";
      const result = await exportPortfolioCsv(String(portfolioId));
      if (result.error) return json(404, { error: result.error });
      return text(200, result.csv, {
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="${result.filename}"`,
    ...
    if (method === "GET" && pathname === "/api/portfolios/export-analysis") {
      const portfolioId = query.id || query.portfolioId || "default";
      const result = await exportPortfolioAnalysisCsv(String(portfolioId));
      if (result.error) return json(404, { error: result.error });
      return text(200, result.csv, {
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": `attachment; filename="${result.filename}"`,
    ...
    if (method === "GET" && pathname === "/api/portfolios") {
      const portfolioId = query.id || query.portfolioId || "default";
      const data = await getPortfolioAnalysis(String(portfolioId));
      return json(200, wrapResponse({
        ...data,
        persistence: getPersistenceInfo(),
      }, {
        source: "Yahoo Finance Chart API + user portfolio store",
        dataType: "mixed",
        confidence: data.available ? 88 : 0,
        disclaimer: "P&L uses verified live prices and user-entered cost basis only. Not investment advice.",
      }));
    }

    const pfImportMatch = pathname.match(/^\/api\/portfolios\/([^/]+)\/import$/);
    if (method === "POST" && pfImportMatch) {
      const csvText = body?.csv || body?.text || "";
      const replace = body?.replace === true;
      const result = await importPortfolioCsv(
        decodeURIComponent(pfImportMatch[1]),
        csvText,
        { replace }
      );
      if (result.error) return json(400, result);
      return json(200, wrapResponse(result, {
        source: "ABC Portfolio CSV import",
        dataType: "factual",
        confidence: 100,
      }));
    }

    if (method === "POST" && pathname === "/api/portfolios") {
      const portfolio = await createPortfolio(body?.name);
      return json(201, wrapResponse({ portfolio }, { source: "ABC Portfolio Store", dataType: "factual", confidence: 100 }));
    }

    const pfHoldMatch = pathname.match(/^\/api\/portfolios\/([^/]+)\/holdings$/);
    if (method === "POST" && pfHoldMatch) {
      const result = await upsertHolding(decodeURIComponent(pfHoldMatch[1]), body || {});
      if (result.error) return json(400, { error: result.error });
      return json(200, wrapResponse(result, { source: "ABC Portfolio Store", dataType: "factual", confidence: 100 }));
    }

    const pfDelHoldMatch = pathname.match(/^\/api\/portfolios\/([^/]+)\/holdings\/([^/]+)$/);
    if (method === "DELETE" && pfDelHoldMatch) {
      const result = await removeHolding(
        decodeURIComponent(pfDelHoldMatch[1]),
        decodeURIComponent(pfDelHoldMatch[2])
      );
      if (result.error) return json(404, { error: result.error });
      return json(200, wrapResponse(result, { source: "ABC Portfolio Store", dataType: "factual", confidence: 100 }));
    }
    ...
    if (method === "GET" && chartMatch) {
      ...
      const resolved = resolveChartRequest(query.range || query.period);
      // Allow explicit interval override only when safe (never invent bars)
      const interval =
        query.interval && ["1m", "2m", "5m", "15m", "30m", "60m", "1h", "1d", "1wk", "1mo"].includes(String(query.interval))
          ? String(query.interval)
          : resolved.interval;
      const range = resolved.range;
      let chart;
      try {
        chart = await fetchChart(symbol, interval, range);
      } catch (err) {
        logApiFailure("Yahoo Finance", `/v8/finance/chart/${symbol}`, err, { range, interval });
        ...
        return json(intelErr.httpStatus, wrapIntelligenceError(intelErr, {
          chartMeta: buildChartMeta({ symbol, range, interval, candleCount: 0, rejected: 0 }),
        }));
      }

      const sanitized = sanitizeCandles(chart.candles, { interval });
      ...
      const indicators = candles.length >= 5 ? computeIndicators(candles) : { latest: {}, series: {}, methodology: {} };
    ...
    if (method === "GET" && pathname === "/api/fii-dii") {
      ...
      const fiiNet = data.fii?.netValue;
      const diiNet = data.dii?.netValue;
      let smartMoneyDirection = null;
      if (fiiNet == null || !Number.isFinite(Number(fiiNet))) {
        smartMoneyDirection = null; // never invent BUY/SELL when net is missing
      } else if (fiiNet > 0) {
        smartMoneyDirection = "FII_NET_BUY";
      } else if (fiiNet < 0) {
        smartMoneyDirection = "FII_NET_SELL";
      } else {
        smartMoneyDirection = "FII_NET_NEUTRAL";
      }
      const flowConfidence =
        fiiNet != null && diiNet != null ? 95 : fiiNet != null || diiNet != null ? 70 : 0;
      return json(200, wrapResponse({ ...data, trends, history: history.slice(0, 10), alerts, institutionalAnalysis: {
        smartMoneyDirection,
        smartMoneyAvailable: smartMoneyDirection != null,
    ...
    if (method === "GET" && researchMatch) {
      ...
      void logRecommendation({ type: "research_report", symbol: report.symbol, confidence: report.aiConclusion.confidenceScore, signal: report.technicalAnalysis.trend });
    ...
    if (method === "POST" && pathname === "/api/copilot") {
      ...
      let result;
      try {
        result = await answerCopilotQuery(rawQuery);
      } catch (err) {
        logApiFailure("Copilot", pathname, err, { query: rawQuery.slice(0, 80) });
        return json(503, wrapResponse({
      ...
      if (!result.ok) {
        return json(result.status, wrapResponse(result.body, {
          source: "ABC Copilot — verified data only",
      ...
      return json(200, wrapResponse(result.body, {
        source: "ABC Copilot — verified data only",
        dataType: result.body.dataType,
        confidence: result.body.confidence,
        lastUpdated: result.body.fetchedAt,
      }));
    }

    if (method === "GET" && pathname === "/api/copilot/suggestions") {
      const status = getCopilotStatus();
      return json(200, wrapResponse({
        suggestions: SUGGESTED_QUERIES,
        maxQueryLength: MAX_COPILOT_QUERY_LEN,
        ...status,
      }, { source: "ABC Copilot", dataType: "factual", confidence: 100 }));
    }

    if (method === "GET" && pathname === "/api/copilot/status") {
      return json(200, wrapResponse(getCopilotStatus(), {
        source: "ABC Copilot",
        dataType: "factual",
        confidence: 100,
      }));
    ...
    if (method === "GET" && researchGenMatch) {
      ...
      void logRecommendation({ type: "research_report", symbol: result.report.symbol, confidence: result.report.confidence });
