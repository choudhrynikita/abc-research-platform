...
const { fetchIpoDashboard, fetchIpoDetail, enrichOpenWithSubscription } = require("./nse-ipo");
...
const strategiesPath = dataPath("strategies.json");
...
async function handleApi({ method, pathname, query = {}, body = null }) {
  try {
    if (method === "GET" && pathname === "/api/health") {
      return json(200, { status: "ok", project: "abc", version: "2.0.0", framework: "nextjs", timestamp: new Date().toISOString() });
    }
    ...
    if (method === "GET" && pathname === "/api/quotes") {
      ...
      return json(200, { source: "Yahoo Finance v8 Chart API (free)", fetchedAt: new Date().toISOString(), count: quotes.length, quotes });
    }
    ...
    if (method === "GET" && pathname === "/api/strategies/alignment") {
      return json(200, buildAlignmentReport(readStrategies(), await getNiftyPrediction()));
    }

    const strategyIdMatch = pathname.match(/^\/api\/strategies\/([^/]+)$/);
    if (method === "GET" && strategyIdMatch && strategyIdMatch[1] !== "alignment") {
      const strategy = readStrategies().find((s) => s.id === strategyIdMatch[1]);
    ...
    if (method === "GET" && researchMatch) {
      ...
      logRecommendation({ type: "research_report", symbol: report.symbol, confidence: report.aiConclusion.confidenceScore, signal: report.technicalAnalysis.trend });
    ...
    if (method === "GET" && csvMatch) {
      ...
      if (type === "nifty500") {
        ...
        csv = toCsv(readStrategies(), [{ key: "name", label: "Name" }, { key: "date", label: "Date" }, { key: "expiry", label: "Expiry" }, { key: "bias", label: "Bias" }, { key: "status", label: "Status" }, { key: "entry", label: "Entry" }, { key: "target", label: "Target" }, { key: "stopLoss", label: "StopLoss" }]);
    ...
    if (method === "POST" && pathname === "/api/copilot") {
      ...
      logRecommendation({ type: "copilot", query: queryText, confidence: response.confidence });
      return json(200, wrapResponse(response, { source: "ABC Copilot", dataType: response.dataType, confidence: response.confidence }));
    }

    if (method === "GET" && pathname === "/api/audit") {
      return json(200, wrapResponse({ entries: readLog() }, { source: "ABC Audit Log", dataType: "factual", confidence: 100 }));
    }
    ...
    if (method === "GET" && pathname === "/api/ipo-alerts") {
      return json(200, wrapResponse(readAlerts(), { source: "ABC IPO Alerts", dataType: "factual", confidence: 100 }));
    }

    if (method === "POST" && pathname === "/api/ipo-alerts/preferences") {
      const prefs = updatePreferences(body || {});
    ...
    if (method === "GET" && researchGenMatch) {
      ...
      logRecommendation({ type: "research_report", symbol: result.report.symbol, confidence: result.report.confidence });
    ...
    if (method === "GET" && pathname === "/api/report-center") {
      return json(200, wrapResponse({ reports: listReports() }, { source: "ABC Report Center", dataType: "factual", confidence: 100 }));
    }

    const exportMatch = pathname.match(/^\/api\/report-center\/([^/]+)\/export\/(.+)$/);
    if (method === "GET" && exportMatch) {
      const entry = getReport(exportMatch[1]);
    ...
    if (method === "GET" && ipoSymbolMatch && ipoSymbolMatch[1] !== "dashboard") {
      const detail = await fetchIpoDetail(ipoSymbolMatch[1]);
