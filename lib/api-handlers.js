    ...
    if (method === "POST" && pathname === "/api/copilot") {
      ...
      try {
        ...
        if (symbolMatch || queryText.includes("analyze")) {
          const sym = symbolMatch ? symbolMatch[0].toUpperCase() : "RELIANCE";
          const report = await buildResearchReport(sym);
          const rsi = report.technicalAnalysis?.rsi;
          const rsiPart = rsi != null && Number.isFinite(rsi)
            ? `RSI ${rsi.toFixed(1)} (verified technical model).`
            : `${MESSAGES.UNAVAILABLE_GENERAL} for RSI.`;
          const trend = report.technicalAnalysis?.trend || MESSAGES.UNAVAILABLE_GENERAL;
          response = {
            answer: `${report.companyName || sym}: Technical trend is ${trend}. ${rsiPart} See full research report for verified details only.`,
            dataType: classifyDataType({ verified: true, isModel: true }),
            confidence: report.aiConclusion?.confidenceScore ?? null,
            report,
            ...policyMeta,
          };
          ...
          const signal = prediction?.ensembleSignal || MESSAGES.UNAVAILABLE_GENERAL;
          const pricePart = price != null && Number.isFinite(price)
            ? `Current price ${price} (Yahoo Finance).`
            : `${MESSAGES.UNAVAILABLE_GENERAL} for current price.`;
          const targetPart = weeklyTarget != null && Number.isFinite(weeklyTarget)
            ? `Weekly model target ${weeklyTarget}.`
            : `${MESSAGES.UNAVAILABLE_GENERAL} for weekly target.`;
          response = {
            answer: `Nifty ensemble signal: ${signal}. ${pricePart} ${targetPart} This is a probability-based forecast, not a certainty.`,
            dataType: classifyDataType({ verified: price != null, isForecast: true }),
            confidence: prediction?.predictions?.monthly?.confidence ?? null,
            data: prediction,
            ...policyMeta,
          };
          ...
          if (fiiNet == null && diiNet == null) {
            const intelErr = mapApiFailure(new Error("empty fii-dii response"), { action: "Retry during NSE market hours." });
            return json(intelErr.httpStatus, wrapIntelligenceError(intelErr));
          }
          response = {
            answer: `FII net ${fiiNet ?? "N/A"} Cr, DII net ${diiNet ?? "N/A"} Cr on ${date || "N/A"} (NSE source).`,
            dataType: classifyDataType({ verified: fiiNet != null || diiNet != null }),
            confidence: fiiNet != null && diiNet != null ? 95 : 60,
            data: fiiDii,
            ...policyMeta,
          };
          ...
          const avgChange = banks[0]?.avgChange;
          const changePart = avgChange != null && Number.isFinite(avgChange)
            ? `${avgChange}%`
            : "N/A";
          response = {
            answer: `Financial Services sector avg change: ${changePart} (sample of ${dash.marketBreadth?.sampleSize ?? "N/A"} tracked stocks).`,
            dataType: classifyDataType({ verified: avgChange != null }),
            confidence: avgChange != null ? 80 : 40,
            data: dash.sectorAnalysis,
            ...policyMeta,
          };
        } else {
          response.answer = "I can analyze stocks (e.g. 'Analyze Reliance'), NIFTY outlook, FII/DII activity, or sector rotation. All responses use verified live data only — never estimates.";
          response.dataType = DATA_CLASSIFICATION.OPINION;
          response.confidence = 100;
        }
      } catch (err) {
        logApiFailure("Copilot", pathname, err, { query: queryText.slice(0, 80) });
        const intelErr = mapApiFailure(err, { action: "Retry after verifying data source connectivity." });
        return json(intelErr.httpStatus, wrapIntelligenceError(intelErr));
      }
