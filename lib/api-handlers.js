...
const { buildFundsDashboard, searchFundsQuery } = require("./funds-etf");
const { buildCommoditiesDashboard, buildCommoditiesFallback } = require("./commodities-engine");
...
const { buildNewsDesk } = require("./news-engine");

/** Storage mode metadata for health/ops — no user portfolio or broker state. */
function getPersistenceInfo() {
  ...
  return {
    storageMode,
    description: modes[storageMode] || storageMode,
    modes,
...
const API_ROUTES = [
  ...
  { method: "GET", path: "/api/defaults", description: "Default quote symbols for legacy tools" },
  ...
  { method: "GET", path: "/api/funds/dashboard", description: "Mutual funds and index ETFs (Nifty BeES)" },
  { method: "GET", path: "/api/funds/search", description: "Search AMFI scheme NAVs" },
  { method: "GET", path: "/api/commodities/dashboard", description: "Gold, silver, crude, gas, copper, USDINR plans" },
  ...
  { method: "GET", path: "/api/persistence", description: "JSON store storage mode (KV / local / tmp)" },
  { method: "GET", path: "/api/shareholding/:symbol", description: "NSE verified shareholding pattern" },
  { method: "GET", path: "/api/news", description: "Latest India share-market headlines + desk impact notes" },
  { method: "GET", path: "/api/news/:symbol", description: "Headlines filtered to a listed symbol" },
];
...
const cache = {
  ...
  institutionalIpoDashboard: { value: null, expiresAt: 0 },
};
...
async function handleApi({ method, pathname, query = {}, body = null, authHeader = null }) {
  try {
    ...
    if (method === "GET" && pathname === "/api/persistence") {
      return json(200, wrapResponse(getPersistenceInfo(), {
    ...
    if (method === "GET" && pathname === "/api/news") {
      const desk = await buildNewsDesk({
        symbol: query.symbol || null,
        refresh: query.refresh === "true" || query.refresh === "1",
      });
      return json(desk.available ? 200 : 503, desk);
    }

    const newsSymbolMatch = pathname.match(/^\/api\/news\/([^/]+)$/);
    if (method === "GET" && newsSymbolMatch) {
      const desk = await buildNewsDesk({
        symbol: decodeURIComponent(newsSymbolMatch[1]),
        refresh: query.refresh === "true" || query.refresh === "1",
      });
      return json(desk.available ? 200 : 503, desk);
    }
    ...
    if (method === "GET" && pathname === "/api/nifty-strategy/dashboard") {
      ...
      const fresh = force ? null : await getDashboardCache("nifty-strategy", 10 * 60 * 1000);
      if (fresh?.data?.top10) {
        return json(200, wrapResponse({ ...fresh.data, cacheHit: true }, {
          source: fresh.data.source,
          dataType: "mixed",
          confidence: fresh.data.chainStatus?.verified ? 85 : 55,
          disclaimer: fresh.data.disclaimer,
          lastUpdated: fresh.data.refreshedAt,
        }));
      }
      try {
        const data = await buildInstitutionalStrategyDashboard();
        await setDashboardCache("nifty-strategy", data);
        ...
        const stale = await getDashboardCache("nifty-strategy", 24 * 60 * 60 * 1000);
        if (stale?.data?.top10) {
          return json(200, wrapResponse({
            ...stale.data,
            cacheHit: true,
            staleFallback: true,
            loadWarning: err.message,
          }, {
            source: stale.data.source,
            dataType: "mixed",
            confidence: 50,
            disclaimer: stale.data.disclaimer,
            lastUpdated: stale.data.refreshedAt,
          }));
        }
        return json(200, wrapResponse({
          available: false,
          ...
          top10: [],
          message: err.message || "Strategy dashboard timed out while loading NSE data",
        }, { source: "ABC", dataType: "unavailable", confidence: 0 }));
      }
    }

    if (method === "GET" && pathname === "/api/equity-fno/dashboard") {
      const force = query.refresh === "1";
      const fresh = force ? null : await getDashboardCache("equity-fno", 10 * 60 * 1000);
      if (fresh?.data?.top10) {
        return json(200, wrapResponse({ ...fresh.data, cacheHit: true }, {
          source: fresh.data.source,
          dataType: "mixed",
          confidence: (fresh.data.executiveSummary?.chainsVerified ?? 0) > 0 ? 82 : 52,
          disclaimer: fresh.data.disclaimer,
          lastUpdated: fresh.data.refreshedAt,
        }));
      }
      try {
        ...
        await setDashboardCache("equity-fno", data);
        ...
        const stale = await getDashboardCache("equity-fno", 24 * 60 * 60 * 1000);
        if (stale?.data?.top10) {
          return json(200, wrapResponse({
            ...stale.data,
            cacheHit: true,
        ...
        return json(200, wrapResponse({
          ...
          top10: [],
          message: err.message || "Equity F&O dashboard timed out while loading NSE data",
        }, { source: "ABC", dataType: "unavailable", confidence: 0 }));
      }
    }

    if (method === "GET" && pathname === "/api/funds/dashboard") {
      const force = query.refresh === "1";
      const fresh = force ? null : await getDashboardCache("funds-etf", 30 * 60 * 1000);
      if (fresh?.data?.etfs) {
        return json(200, wrapResponse({ ...fresh.data, cacheHit: true }, {
          source: fresh.data.source,
          dataType: "factual",
      ...
      try {
        const data = await buildFundsDashboard();
        await setDashboardCache("funds-etf", data);
        return json(200, wrapResponse(data, {
          source: data.source,
          dataType: "factual",
          confidence: 80,
        ...
        const stale = await getDashboardCache("funds-etf", 24 * 60 * 60 * 1000);
        if (stale?.data?.etfs) {
          return json(200, wrapResponse({
            ...stale.data,
            cacheHit: true,
        ...
        return json(200, wrapResponse({
          ...
          etfs: [],
          featured: [],
          message: err.message || "Funds desk could not load AMFI / ETF quotes",
        }, { source: "AMFI/Yahoo", dataType: "unavailable", confidence: 0 }));
      }
    }

    if (method === "GET" && pathname === "/api/funds/search") {
      const q = String(query.q || query.query || "").trim();
      if (q.length < 2) {
        return json(200, wrapResponse({ query: q, results: [] }, { source: "AMFI", dataType: "factual" }));
      }
      try {
        const data = await searchFundsQuery(q);
        return json(200, wrapResponse(data, { source: "AMFI NAVAll", dataType: "factual", lastUpdated: data.navDate }));
      } catch (err) {
        return json(200, wrapResponse({ query: q, results: [], message: err.message }, { source: "AMFI", dataType: "unavailable", confidence: 0 }));
      }
    }

    if (method === "GET" && pathname === "/api/commodities/dashboard") {
      const force = query.refresh === "1";
      const fresh = force ? null : await getDashboardCache("commodities-v2", 20 * 60 * 1000);
      if (fresh?.data?.strategies?.length) {
        return json(200, wrapResponse({ ...fresh.data, cacheHit: true }, {
          source: fresh.data.source,
          dataType: "mixed",
          confidence: 72,
      ...
      try {
        const data = await buildCommoditiesDashboard();
        await setDashboardCache("commodities-v2", data);
        return json(200, wrapResponse(data, {
          source: data.source,
          dataType: "mixed",
          confidence: 72,
        ...
        const stale = await getDashboardCache("commodities-v2", 24 * 60 * 60 * 1000);
        if (stale?.data?.strategies?.length) {
          return json(200, wrapResponse({
            ...stale.data,
            cacheHit: true,
            staleFallback: true,
            loadWarning: err.message,
          }, { source: stale.data.source, dataType: "mixed", confidence: 45, lastUpdated: stale.data.refreshedAt }));
        }
        return json(200, wrapResponse({
          ...buildCommoditiesFallback(err.message),
          available: false,
        }, { source: "Named tickets without live tape", dataType: "unavailable", confidence: 20 }));
      }
    }
    ...
    if (method === "GET" && pathname === "/api/ipo/terminal") {
      try {
        ...
        setCached("institutionalIpoDashboard", data, 24 * 60 * 60 * 1000);
        return json(200, wrapResponse(data, {
          source: data.source,
          dataType: "factual",
          confidence: 92,
        ...
        const lastVerified = cache.institutionalIpoDashboard.value?.refreshedAt || null;
        const outage = {
          available: false,
          dataStatus: "unavailable",
          message: "NSE IPO data is temporarily unavailable. Retry shortly.",
          lastVerifiedAt: lastVerified,
          source: "NSE India IPO APIs",
          sections: { open: [], upcoming: [], listed: [] },
          counts: { open: 0, upcoming: 0, listed: 0 },
        };
        logApiFailure("NSE India IPO APIs", "/api/ipo/terminal", error, { lastVerifiedAt: lastVerified });
        return json(200, wrapResponse(outage, {
          source: outage.source,
          dataType: "unavailable",
          confidence: 0,
          lastUpdated: lastVerified || new Date().toISOString(),
          extra: { verified: false, code: "UPSTREAM_UNAVAILABLE" },
        }));
      }
    }
