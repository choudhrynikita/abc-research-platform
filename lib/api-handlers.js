...
async function handleApi({ method, pathname, query = {}, body = null, authHeader = null }) {
  try {
    ...
    if (method === "GET" && pathname === "/api/commodities/dashboard") {
      const force = query.refresh === "1";
      const fresh = force ? null : await getDashboardCache("commodities", 20 * 60 * 1000);
      ...
      try {
        ...
        await setDashboardCache("commodities", data);
        return json(200, wrapResponse(data, {
          source: data.source,
          dataType: "mixed",
          confidence: 72,
        ...
        const stale = await getDashboardCache("commodities", 24 * 60 * 60 * 1000);
