...
function snapshotFromHistory(entry) {
  // Accept net-only rows (buy/sell may be null) — never drop verified net activity
  const hasFii = entry.fiiBuy != null || entry.fiiSell != null || entry.fiiNet != null;
  const hasDii = entry.diiBuy != null || entry.diiSell != null || entry.diiNet != null;
  return {
    date: entry.date,
    fii: hasFii
    ...
    raw: [],
    dataStatus: "cached",
    fetchedAt: entry.recordedAt || null,
  ...
  const sections = [
    {
      ...
      content: `FII daily net ${live.fii?.netValue != null ? fmtCr(live.fii.netValue) : UNAVAILABLE_FIELD}; DII daily net ${live.dii?.netValue != null ? fmtCr(live.dii.netValue) : UNAVAILABLE_FIELD}. Smart money: ${intelligence.smartMoneyDirection}. ${usedCache ? "Using last verified stored session." : "Verified live NSE session."}`,
    },
    {
