 ...
 * Intraday uses sub-daily intervals. Daily bars stay on 1d through 1y.
 * Multi-year ranges step up to weekly/monthly so candles stay readable
 * instead of overlapping into a solid smear on Max / 5Y.
 */
function resolveChartRequest(rangeInput) {
  ...
  else if (range === "1mo" || range === "3mo" || range === "6mo" || range === "1y" || range === "ytd") {
    interval = "1d";
  } else if (range === "2y" || range === "5y" || range === "10y") {
    interval = "1wk";
  } else if (range === "max") {
    interval = "1mo";
  } else {
