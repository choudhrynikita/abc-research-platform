 ...
 * light=true: index-only overview (used by institutional Top 50 which runs its own full screen)
 */
async function buildNifty500Dashboard(opts = {}) {
  ...
  if (!light) {
    // Bounded concurrency — full NIFTY 500 must not open 500 parallel sockets
    const quotes = await mapPool(constituentsMeta, 12, fetchConstituentQuote);
    // Technical enrich only liquid subset to stay within serverless budgets
    const withPrice = quotes.filter((q) => q.price != null);
    const enrichTargets = [...withPrice]
      .sort((a, b) => (b.volume ?? 0) - (a.volume ?? 0))
      .slice(0, Math.min(80, withPrice.length));
    const enriched = await mapPool(enrichTargets, 8, (q) =>
      enrichConstituentTechnicals(q, fetchChart)
    );
    const quoteMap = new Map(enriched.map((q) => [q.symbol, q]));
    mergedQuotes = quotes.map((q) => quoteMap.get(q.symbol) || q);
    valid = mergedQuotes.filter((q) => q.price != null);
    advances = valid.filter((q) => (q.changePercent ?? 0) > 0).length;
    declines = valid.filter((q) => (q.changePercent ?? 0) < 0).length;
  ...
  return {
    marketOverview: {
      ...
      sampleUniverse: light,
      note: light
        ? `NIFTY 500 index via Yahoo Finance ^CRSLDX; constituent universe seed has ${constituentsMeta.length} names. Full Top 50 ranking uses institutional multi-phase screen.`
        : `NIFTY 500 index via Yahoo Finance ^CRSLDX; constituent breadth from ${constituentsMeta.length}-name reference universe (NSE list). Technical enrich limited to top liquid names for performance.`,
    ...
    technicalScreen: light ? [] : runTechnicalScreen(mergedQuotes),
    fundamentalScreen: light ? [] : runFundamentalScreen(mergedQuotes),
    sectorAnalysis: {
