...
/** Markdown shareholding block from verified NSE/Yahoo fields — never invents percents. */
function formatShareholdingMarkdown(sh) {
  if (!sh || typeof sh !== "object") {
    return unavailable("Requires NSE/BSE shareholding feed — never estimated.");
  }
  const rows = [
    ["Promoter", sh.promoter],
    ["FII / FPI", sh.fii],
    ["DII", sh.dii],
    ["Mutual Funds", sh.mutualFunds],
    ["Institutional", sh.institutional],
    ["Public", sh.public],
  ];
  const any = rows.some(([, field]) => val(field) != null);
  const lines = rows.map(([label, field]) => {
    const raw = val(field);
    if (raw == null) {
      return `- **${label}:** ${unavailable(field?.reason || field?.message)}`;
    }
    return `- **${label}:** ${fmtPctFraction(raw)}`;
  });
  if (sh.asOf) lines.push(`- **As of:** ${sh.asOf}`);
  if (sh.source) lines.push(`- **Source:** ${sh.source}`);
  if (!any) {
    return `${unavailable(sh.message || "Requires NSE/BSE shareholding feed — never estimated.")}\n${lines.join("\n")}`;
  }
  if (sh.message) lines.unshift(sh.message);
  return lines.join("\n");
}
...
function definitionAnswer(intent, query) {
  ...
  const defs = {
    ...
    roce: {
      ...
      body: "ROCE measures EBIT relative to capital employed. ABC computes ROCE only when Yahoo statements provide verified EBIT and capital employed (assets − current liabilities, or equity + long-term debt). Missing inputs show Data Unavailable — never estimated.",
    },
    peg: {
      title: "PEG Ratio",
...
function buildResearchSections(report, institutional) {
  ...
  pushMetric("ROCE", valn.roce ?? fund.roce, fmtPctFraction);
  ...
  // Shareholding — NSE SHP when merged into research/fundamentals
  const sh = (data || base).shareholding || base.fundamentals?.shareholding || {};
  lines.push(`### Shareholding Pattern\n${formatShareholdingMarkdown(sh)}\n`);
  ...
  lines.push(
    `---\n**Sources:** Yahoo Finance Chart API + quoteSummary · NSE Corporate Filings SHP (shareholding) · ABC technical models on verified OHLCV · Peer maps from platform reference files.\n**Policy:** Missing metrics show Data Unavailable — never estimated or fabricated.`
  );
