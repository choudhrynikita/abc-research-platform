...
const STOP_TICKERS = new Set([
  ...
  "BANKING", "PHARMA", "FMCG", "AUTO", "ENERGY", "TELECOM", "IT",
]);
...
function classifyIntent(query) {
  ...
  // Educational definitions when no equity ticker is present
  const isDefQuestion = /\b(what is|define|meaning of|explain|how does)\b/.test(q);
  const mentionsTechMetric = /\b(rsi|macd|sma|ema|bollinger|atr|adx|vwap|stochastic)\b/.test(q);
  const mentionsFundMetric = /\b(pe ratio|p\/e|pb ratio|p\/b|roe|roa|roce|peg|ev\/ebitda|dividend yield|market cap|fcf)\b/.test(q);
  if (!symbols.length && (isDefQuestion || mentionsTechMetric || mentionsFundMetric)) {
...
function definitionAnswer(intent, query) {
  ...
  else if (/\b(pe|p\/e)\b/.test(q)) key = "pe";
  ...
  return {
    answer:
      "## Financial Concept\n\nAsk about a specific metric (e.g. RSI, MACD, P/E, ROE) or a ticker (e.g. \"Analyze TCS\"). Live figures require a company or market query and verified data sources.",
    dataType: DATA_CLASSIFICATION.OPINION,
    confidence: 100,
    intent,
    sources: [],
...
async function answerCopilotQuery(rawQuery) {
  ...
  try {
    if (intent.startsWith("definition")) {
      ...
      const symbol = symbols[0] || "RELIANCE.NS";
      result = await answerStockQuery(symbol, intent === "help" ? "research_full" : intent);
    }
  } catch (err) {
    return {
      ok: false,
