...
function buildAnalytics(legs, chain) {
  ...
  return {
    ...
    ivRank: null,
    ivPercentile: null,
    ivNote: "IV rank/percentile requires historical IV series — not estimated",
...
function makeStrategy(base, ctx) {
  ...
  const analytics = buildAnalytics(legs, ctx.chain);

  const t1 = base.targets?.t1;
  const t2 = base.targets?.t2;
  const profitAtT1 = t1 != null && sizing.premiumPerLot != null && netPrem > 0
    ? Number((t1 * (ctx.lotSize ?? 1)).toFixed(2))
    : null;
  const profitAtT2 = t2 != null && sizing.premiumPerLot != null && netPrem > 0
    ? Number((t2 * (ctx.lotSize ?? 1)).toFixed(2))
    : null;

  const rocT1 = profitAtT1 != null && sizing.capitalRequired > 0
    ? Number(((profitAtT1 / sizing.capitalRequired) * 100).toFixed(1))
    : null;
