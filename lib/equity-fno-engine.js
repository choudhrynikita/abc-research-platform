...
function liquidityRating(legs) {
  return rateLiquidityFromLegs(legs);
}
...
function scoreStrategy(strategy, ctx) {
  ...
  if (trend === "BULLISH" && strategy.bias === "Bearish") {
    score -= 18;
    factors.push("Counter-trend vs stock tape");
  } else if (trend === "BEARISH" && strategy.bias === "Bullish") {
    score -= 18;
    factors.push("Counter-trend vs stock tape");
  } else if (trend === "NEUTRAL" && strategy.bias === "Neutral") {
    score += 10;
    factors.push("Fits range-bound tape");
  } else if (trend === "NEUTRAL" && (strategy.bias === "Bullish" || strategy.bias === "Bearish")) {
    score -= 6;
  }

  const rr = strategy.payoff?.maxProfitUnlimited || /^(Long CE|Long PE)$/i.test(strategy.type)
    ? null
...
function makeStrategy(base, ctx) {
  ...
  if (hasLegs) {
    ...
    strategy = applyHonestManagementTargets(strategy);
  } else {
    strategy.payoff = {
      available: false,
...
function generateCandidates(chain, ctx) {
  ...
  if (ceLeg?.premium != null && ctx.trend === "BULLISH") {
  ...
  if (peLeg?.premium != null && ctx.trend === "BEARISH") {
...
function rankTop10(allCandidates, globalContext) {
  const ranked = [...(allCandidates || [])]
    .map((c) => {
      ...
      const scoring = scoreStrategy(c, ctx);
      return {
        ...c,
  ...
  return ranked.map((s, i) => {
    const ctx = { ...globalContext, ...s._ctx };
    const attached = attachEquityFnoDossier(s, ctx);
    const { _ctx, ...rest } = s;
    return {
      ...rest,
