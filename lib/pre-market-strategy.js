...
function makePreMarketStrategy(base) {
  ...
  return {
    ...
    premiumNote: base.premiums?.net != null
      ? "Reference premium from the last verified session close — confirm the current premium and trigger when the next session begins"
      : "Entry premium is not available yet — use the trigger conditions when the next session begins",
  };
}
...
function generateTechnicalSetups(context, instrument = "NIFTY") {
  ...
  return unique
    .slice(0, 10)
    .map((s) => enrichPreMarketFromChain(applyStructuralRiskReward(s, context), chain));
}

function applyStructuralRiskReward(strategy, context) {
  const { price, support, resistance } = context;
...
function enrichPreMarketFromChain(strategy, chain) {
  if (!chain?.available || !strategy.strikes?.length) return strategy;

  const legs = strategy.strikes.map((s) => {
  ...
  return {
    ...
    entryZone: { low: Number((absNet * 0.92).toFixed(2)), high: Number((absNet * 1.08).toFixed(2)) },
    premiumNote:
      "Reference premium from last verified NSE session — confirm trigger after market open",
  };
}
...
function annotateForPlanning(strategy, marketStatus, chainMeta) {
  ...
  return {
    ...
    mode: "planning",
    planningHorizon,
    modeLabel: `${state.label} — latest verified close`,
    premiumNote: hasReferencePremium
      ? "Reference premium from the latest verified session close — confirm the current premium and trigger when the planned session begins"
      : strategy.premiumNote || "Entry premium is not available yet — use the trigger conditions when the planned session begins",
